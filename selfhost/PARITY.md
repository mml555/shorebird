...
| ◐ | **BUILT 2026-08-14, WITH ITS OWN CAVEAT UNMET.** Both patch arms ran non-TTY on `R12`: `patch android --no-confirm` **exit 0**, and `--json patch android` **exit 0** with **`interactive_prompt_required: 0`** (`Published Patch 2!`). **But this row's stated mechanism is still not exercised.** It requires two releases so `release_chooser.dart:80-82` cannot skip the selection prompt — two now exist (`1.6.0+1`, `1.7.0+1`), yet the harness passes `--release-version` on every patch, so the chooser is bypassed by the FLAG regardless of release count. The arm that would exercise it is `patch` with **no** `--release-version` against ≥2 releases, expecting exit 64 naming the prompt (outcome 11) rather than a hang (outcome 12). **OWED, and cheap now the rig stands.** Prior text: **NOT VALIDATED** Fully noninteractive CI patch — same, and the harness must publish **two** releases or it proves less than it appears: `release_chooser.dart:80-82` skips the release-selection prompt entirely when exactly one release exists |
