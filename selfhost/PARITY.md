...
| ◐ | **BUILT 2026-08-13, host-proven with two negative controls** Replacement methods with explicit source parameters — **`G3.7 param-abi`, worth 33.2 %**. `g37_param_abi.sh` 4/4: a one-parameter and a two-parameter target patched and executed through the real producer path; named and optional-positional targets refused at patch time against IDENTICAL release bytes. Contract in patch `0006`, pinned by `c_entrypoint_arity.sh` 8/8. Device gate needs a mint | the replacement's **own** signature, distinct from the call it makes. The entry-point contract is 0-or-1 positional and the receiver already claims the one slot; `9192a594` did not widen it. ~~**This single item is worth more than the entire privacy problem**~~ — **STATUS CORRECTED 2026-08-25, and deliberately NOT re-priced.** Both halves are now closed: the parameter ABI is proven on host and device, and private-library scope is no longer open either (`G3.6b`/`G3.6e`, device-closed the same week). So the comparison this sentence draws is **historical**: it ranked two blockers that were both open, measured when privacy was 9/10 and parameters 6/10 in Phase 0. **The `33.2 %` figure and the "worth more than privacy" ranking are historical evidence, not current-ranking evidence** — and no replacement percentage is invented here, because P1.5 has no valid corpus yet (`ROADMAP.md`'s P1.5 banner). See the measurement above for what the numbers meant when they were taken |
...
| **`G3.6e resolve-in-library`** | thread `resolveInLibrary` through dart2bytecode | **THE BOUNDARY IS NOW MEASURED, AND ONE CONTROL FAILED — 2026-08-25.** `probes/p1_private_scope_controls.sh` scores what the flag REFUSES, which had never been tested: no flag, the wrong app library, a second app library's private, a library absent from the dill — all correctly refused. **But `--resolve-private-names-in-library dart:core` granted the compiled source dart:core's private namespace and `_GrowableList` compiled**, refused under an app library in the same probe's control arm. `0005` guarded on `isDartLib`, which is the library being COMPILED, not the one the option NAMES — and its own comment asserted the invariant it did not enforce, so only a negative control could tell. **Closed in `0005` (loud `StateError`) and independently in the producer (refuses a `dart:` target library, mutation-checked test); the CFE half needs a cell rebuild to take effect, the producer half ships with the CLI.** Product exposure was nil throughout: the producer only ever passes the target's own library. Evidence: `engine/route_b/evidence/p1_private_scope_controls.txt`. Both of its own done-criteria are met by release `31.0.0+1` patch 2 (`c7661317`): the **producer** path, not a hand-written replacement, and a **device** round-trip — `value() => _secret` → `NEW-PRIV` on an iPhone 7. `route_b_producer.dart:175` passes `--resolve-private-names-in-library` only when the manifest grants the member, and per this goal's own failure table a missing visibility mechanism fails at COMPILE time; the patch compiled and ran, so the mechanism was in the path. Host: `probe D` 4/4, `a53029c9`, patch `0005`. **A private METHOD/GETTER call is host-proven only — and as of 2026-08-25 that host proof now covers the shape that matters.** `probes/p1_bind_private_receiver.sh` is GREEN 8/8 at BIND time on the idiomatic private-State shape: a patch to a method of a conventionally PRIVATE class binds and executes while its body reads a private FIELD, reads a private GETTER and calls a private METHOD (`NEW-FLDF` / `NEW-GETG` / `NEW-MTHM`). Compile-level scoring cannot see any of this — a `dynamic` receiver accepts every member name with no privacy test — so this is the first evidence for the shape Phase 0 said dominates, and the shape release `31.0.0+1` did **not** cover (that was a private field on a PUBLIC class). Evidence: `engine/route_b/evidence/p1_bind_private_receiver.txt`. **DEVICE ARM CLOSED 2026-08-25** — `fixtures/privatestate_app`, release `1.0.1+1` patch 2 on cell `93a3756…`, iPhone 7: a patch to `_FooState.target`, a public method of a conventionally PRIVATE Flutter State class, rendered **`NEW-FLD-GET-MTH`** on hardware — a granted private FIELD, private GETTER and private METHOD call, all three, with `control=CTL` unmoved. Receipt names the exact target, `uep_post_is_interpret_call=1` with `fn_uep_post == interpret_call_ep`, construction withheld (0 unnamed-ctor grants). **Rollback closed the same day**: patch withdrawn with `rollback=true`, and two tap-launches later `target=OLD` with `next_boot_patch: null` and an empty `patches/` dir — so the specimen reads `release -> OLD`, `patched -> NEW-FLD-GET-MTH`, `rollback -> OLD` with the control unmoved throughout, every row observed on hardware. **It took two attempts at the patched row, and the first is the more useful one:** the lowering inserted a bare receiver prefix inside a simple `$identifier` interpolation, so the device rendered `NEW-Instance of '_FooState'._field-…` — compiled, attached, executed, plausible, wrong. Fixed in `9125eb13`. `fixtures/privatestate_app/evidence/VERDICT.md` | done at `R3`; no mint needed |
| **`G3.7 param-abi`** | a replacement method may declare **its own parameters** | **HOST-COMPLETE 2026-08-25, including the combination with a private receiver.** `probes/p2_private_receiver_with_args.sh` 4/4 on cell `93a3756…`: a method of a PRIVATE class taking `(String, int)` binds both arguments (`A-L-7`), composes with a granted private field read (`B-L-7-H`), and renders `A-7-L` when the same signature is used in the opposite order — so binding is by POSITION and carries types, not merely arity. A named parameter is refused by the shipped contract. That combination was previously untested: P1 proved a private receiver with NO arguments, G3.7 proved arguments on a PUBLIC receiver, and neither implies the other because the producer prepends the receiver and shifts every source parameter by one. **DEVICE GATE CLOSED 2026-08-25 — required positional ABI is PROVEN on host and on physical iOS**, including a private receiver plus multiple typed arguments plus private-member access in one body: `fixtures/privatestate_app` release `1.0.2+1` patch 1 rendered **`args=NEW-A-7-FLD`** on an iPhone 7, with `sel=_FooState.targetArgs`, built-for == running, `rc=0`, `bc_post=1`, `interp_post=1`, `uep_post_is_interpret_call=1`, and rollback back to `args=OLD` with a pristine updater. `target=OLD` held throughout — the other method of the SAME private class was untouched. **Named and optional positional remain deliberately unsupported and refused before publication**, so neither was ever sent to the phone. Evidence: `engine/route_b/evidence/p2_private_receiver_with_args.txt`, `fixtures/privatestate_app/evidence/P2_VERDICT.md`. Prior: **BUILT 2026-08-13.** Feasibility is no longer inferred: patch `0006` widens the contract from 0-or-1 to any number of REQUIRED positionals, the analyzer refuses exactly what the compiler refuses (`analysisVersion` 8), the producer inserts the receiver in front of a verbatim-copied parameter list, and `g37_param_abi.sh` 4/4 proves arguments arrive **in order and by type** — `int b` = 7 landed in slot `b`, so this is not mere arity. Device gate outstanding: the cell carries `dart2bytecode.aot`, so it needs a mint | mint + `R1` remain |
...
| ✅ | ~~**BUILT** A replacement **can** call an existing private member of the application library — `probe D` 4/4, `a53029c9`. Hand-written replacement; producer path and device gate outstanding~~ **PROVEN — this row was stale, corrected 2026-08-25.** Both of the things it calls outstanding closed on 2026-08-13 and are recorded in this file's own `G3.6b`/`G3.6e` goal rows: the PRODUCER path (not a hand-written replacement) and a DEVICE round-trip, release `31.0.0+1` patch 2, `value() => _secret` → `NEW-PRIV` on an iPhone 7. Scope unchanged: a private **read** is proven; a private METHOD/GETTER call is host-proven only; a private **write** is not claimed |
...
| ✅ | **PROVEN 2026-08-25 — MEMBER reach is gated by the interface, and not merely by retention.** `probes/p1_bind_private_receiver.sh` arm **B4b**: a private member deliberately withheld from the interface but still **dispatchable in the release** — the release calls it through a dynamic receiver, and the baseline prints its value — is **not reachable from a patch**: `NoSuchMethodError: Class '_FooState' has no instance method '_ungrantedKept'. Receiver: Instance of '_FooState'`. So a patch sees the class's members filtered to the GRANTED set, even for a member the release is dispatching by name at that moment. **This is a different axis from the row below**: member reach is gated, CONSTRUCTIBILITY is not |
| 🐞 | **KNOWN GAP, now DIAGNOSED AND WITH A MEASURED FIX — 2026-08-25.** Retaining a private **class** makes it **allocatable from a patch** — the patch constructed `_Dead()` with **no constructor named in the interface**, because a `class:` item covers the implicit public default constructor. A capability grant nobody requested. **Cause (not a bug, a definition):** `callable:` is parsed with `allowStaticDeclarations: true`, and upstream's `_Annotator.visitClass` calls `_visitPublicMembers(node.constructors)` — a class's implicit unnamed constructor is public, so a bare `class:` item annotates it. `dynamic_interface_annotator.dart` is **upstream and unmodified**, so the authority is decided by what **we** emit. **Fix, measured both halves (`probes/p1_dead_allocatability.sh`, `evidence/p1_dead_allocatability.txt`):** name the class's MEMBERS individually — including the methods a patch may target — and emit **no bare `class:` item`**. Then a patch still attaches and reads privates (Cfix3) and construction is refused (Cfix4). The obvious alternative — move private classes to `can-be-used-as-type:` — is **REFUTED**: it grants type identity but a patch cannot ATTACH to a method of the class. **PRICED 2026-08-25 and the retention cost is nil** — member-only vs the current policy is **−0.01%** (toy), **+0.00%** (real Flutter app, TFA), **+0.00%** at 200 private classes; the generated YAML roughly doubles, which is a build-time artifact rather than release bytes (`probes/p1_retention_price.sh`, `evidence/p1_retention_price.txt`). A blanket *"grant every constructor"* policy would cost **+8.35%** at 200 classes, which is the argument for opting constructors in **one at a time** — and that is expressible exactly: `member: ''` grants the unnamed constructor and nothing else. **Not yet implemented in `gen_dynamic_interface.dart`.** Also recorded: an ungranted construction **aborts the process** (`object.cc:5500 unreachable code`, or a named `Unable to find function` from `bytecode_reader.cc`) rather than failing gracefully, so this has to be gated pre-publication |
| ✅ | ~~**NOT BUILT** The producer emits it automatically — **`G3.6b`**, gated on the retention policy below, not merely on the analyzer~~ **BUILT AND DEVICE-PROVEN — stale row, corrected 2026-08-25.** `route_b_producer.dart` emits it automatically and gates it on the release's own capability manifest, exactly as this row asked: the member and its enclosing private class must both be granted, **and** every private identifier in the emitted body must be one of those grants. Device: release `31.0.0+1` patch 2 (2026-08-13). This file's `G3.6b` goal row has said so since; only this detail table lagged |
...
| ◐ | **BUILT 2026-08-14 — CLOSED, and the row below is kept as its before-state because its SCOPE was wrong.** Route B now compiles its kernels with the defines **Flutter** injects, read from Flutter’s own answer rather than reconstructed. **The scope correction first, because it is the finding:** the row below called this a thing that “bites where a program branches on one of those values, which is rarer than branching on a flavor,” and framed it on the flavored fixture. Measured on a **CLEAN `flutter create` app with no flavor and no `--dart-define` at all**, the shipped release still receives **six** defines no Route B kernel had — `FLUTTER_VERSION`, `FLUTTER_CHANNEL`, `FLUTTER_GIT_URL`, `FLUTTER_FRAMEWORK_REVISION`, `FLUTTER_ENGINE_REVISION`, `FLUTTER_DART_VERSION` (plus `FLUTTER_ENABLED_FEATURE_FLAGS` when non-empty). **This was every iOS release, not an edge case.** **Measured with Route B’s OWN analyzer, not argued:** `probes/g41c_injected_defines.sh` **5/5** reports `changed = [package:probeapp/main.dart#main]` between the prepass Route B built and the program Flutter compiles — with **three controls that decide whether that means anything**: identical kernels report `NONE` (the instrument can say “no change”), a recompile of the same arm reports `NONE` (not nondeterminism), and an **ordinary user define — which Route B has always forwarded — produces the IDENTICAL verdict**, which is what makes this the same defect class rather than a curiosity. **Two wrong instruments are recorded in the probe header rather than quietly dropped:** diffing the dynamic interface produced a FALSE PASS (app libraries are whole-library, so the only differing line was the `# Source dill:` comment), and grepping symbol names is useless (they survive in the string table whether or not the body is reachable). **THE SEAM, and why it is a reading rather than a reconstruction:** the values come from the `DART_DEFINES` Flutter itself writes to `ios/Flutter/Generated.xcconfig`, obtained before the prepass by `flutter build ios --config-only`, and **checked after the real build rewrites the same file** — disagreement declines patchability exactly where this class already lands. Reconstruction was rejected on measurement: **`FLUTTER_ENGINE_REVISION` is NOT this release’s engine** — it comes from `engine_stamp.json` `git_revision` (`version.dart:681`), **`11e5695710`** on this cell while Shorebird’s `engine.version` reads **`40eaa0ef`**, so deriving it from `shorebirdEnv.shorebirdEngineRevision` is plausibly and silently wrong; `FLUTTER_FRAMEWORK_REVISION` is truncated to 10 chars; and `FLUTTER_GIT_URL` interpolates a **nullable** getter, so it ships the literal `"null"` where `flutter --version --machine` says `unknown source` — **the two candidate sources disagree by construction**, which is why `--version --machine` was rejected as the seam. **Unreadable is NOT empty:** if Flutter’s answer cannot be read the release declares no retention with a named warning, because compiling the prepass with an empty injected set is the bug itself, done silently. **`FLUTTER_APP_FLAVOR` stays excluded** on the measured casing divergence `f06fa056` closed. **FINGERPRINT UNCHANGED, with the reasoning written down as the row below demanded:** the six are *not* added to `RouteBBuildConfig` — a release and a patch on one pinned cell resolve them identically by construction, so the entry could only ever compare a constant with itself while making every previously recorded release incomparable. `analysisVersion` stays **8**. **THREE NEGATIVE CONTROLS, each confirmed RED in its final form** (kernel threading → 4 red; releaser threading → 1 red; the decline guard → 1 red), **and the vacuity controls stayed GREEN in both states** — `emits nothing extra when no injected map is supplied` (without it the positive test would pass against a hard-coded `FLUTTER_*` list, i.e. against the very reconstruction refused here) and `leaves an ordinary user define untouched by the injected set`. **2504 passed / 1 skipped / 0 failed**; `dart analyze --fatal-warnings lib test` exit 0 with zero errors and zero warnings; format clean. **Host only — earns BUILT, and no release has been cut**; what is owed is a Route B iOS release built with this CLI and a matching patch. **iOS only:** Android hands its defines to Gradle and writes them to no file, and Route B builds kernels only on iOS. `evidence/g41-injected-defines/`. The before-state follows: **INTEGRATION REGRESSION ARM RUN 2026-08-14, and it is labelled as one:** the re-sync G15 handed back is done (rig CLI `ba4e1c02` → `50ed19a7`), and `airgap_app` **release 95 (`40.0.0+1`) + patch 1** published first try on cell `40eaa0ef`. **It does NOT upgrade this row and cannot:** that fixture reads none of the six, so its kernels are byte-identical with and without them. What it DOES establish is narrower and real — the pre-prepass `--config-only` read succeeded on a real project (retention was declared; the failure path produces no interface at all), the post-build re-read AGREED (`buildConfig` populated, artifacts captured), and — **the canonical statement, to be quoted rather than paraphrased** — **the release-time agreement check passed with the injected defines threaded into the prepass/import-kernel path, `release_import.dill` was RETAINED, and the patch subsequently BOUND against that import kernel without refusal.** **The build-config fingerprint is a SEPARATE, WEAKER fact and must not be folded into that sentence:** it compared an empty user-define set against an empty one and therefore validated nothing about the injected defines — it cannot, since they are deliberately outside the fingerprint. **TWO LIVING DEBTS, both SHARED rather than this lane's:** (a) **cross-release semantic drift** — releases **≤95** and everything cut after differ in whether the injected defines participate in Route B analysis, the difference is invisible in the fingerprint and the building CLI revision is recorded in no artifact, so any cross-boundary comparison carries two variables with the CLI silent; (b) **rig CLI provenance is shared experimental state, not incidental tooling** — the exact `~/.shorebird` snapshot was load-bearing in two lanes in one session, so *what CLI is installed, who owns it, and when it may be re-synced* must be claimed and released like the engine stamp. Engine stamp unmoved on both measures across the whole run. Cell audit reported its **one expected finding** — the platform-dill split, G15's `82e455c8` — **recorded, not suppressed**. **Still BUILT: an upgrade needs a DISCRIMINATING fixture whose REACHABLE program consumes one of the six observably**, reachable being load-bearing since retention is not reachability. `evidence/g41-injected-defines/RELEASE_40_INTEGRATION_ARM.md`. **THE DISCRIMINATING FIXTURE EXISTS 2026-08-15, and it found a SECOND LINK BROKEN — see the row below.** `selfhost/fixtures/injected_define_app` is a reachable program whose behaviour depends on `FLUTTER_VERSION`, and `probes/g41d_injected_define_patch.sh` (**10/10**) splits the question this row was closing into two links: **link 1, ANALYSIS — fixed, and confirmed more strongly than this row claimed:** Route B's kernel compiled with the threading is **BYTE-IDENTICAL** to the one Flutter compiles (`a6b46303…`), with a two-part negative control so it cannot pass on an incidental difference. **Link 2, REPLACEMENT — still broken**, and that is the new row below. **TWO INSTRUMENT FINDINGS, both caught by controls and both of which invalidated a draft of the probe:** `gen_kernel --aot` does **NOT** tree-shake (both branches are `reachable: yes` in both kernels; TFA runs at `gen_snapshot`), so *which symbol is live* is not a kernel-level observable at all; and **`route_b_analyze`'s `changed` does not see a constant-only body difference** — `-DFLUTTER_VERSION=zzz` vs the real value reports `NONE` while the `.dill`s differ by 24 bytes, so an earlier draft's link-1 arms **passed for the wrong reason**. `g41c` is unaffected and remains the analyzer-level proof (it puts the branch in `main`, where the difference IS seen); `g41d` proves link 1 at the byte layer. **Still BUILT — no release cut from this fixture and no device run**, and the device arm waits for a clean rig hand-back exactly as the integration arm did. |
| ☐ | ~~**KNOWN GAP, measured 2026-08-14 while closing the row above — Flutter injects defines that Route B's kernels never receive.** The same `DART_DEFINES` line the expansion is checked against carries **`FLUTTER_VERSION`, `FLUTTER_CHANNEL`, `FLUTTER_GIT_URL`, `FLUTTER_FRAMEWORK_REVISION`, `FLUTTER_ENGINE_REVISION`, `FLUTTER_DART_VERSION`** (`flutter_command.dart` `_addFlutterVersionToDartDefines`, and `_addFeatureFlagsToDartDefines` for `kEnabledFeatureFlags`), while `RouteBReleaseKernelBuilder.forwardedArgs` forwards only `--dart-define=` and `--enable-experiment=`. **So the prepass and import kernels compile without them and the shipped kernel has them** — the same *different-program* class as the flavor-casing divergence `6ae04dc7`/`f06fa056` closed, with the key threading fixed and a whole family of keys still missing. Measured, not argued: the flavored fixture's `ios/Flutter/Generated.xcconfig:12` decodes to `FLUTTER_APP_FLAVOR=foo`, `FLUTTER_VERSION=3.44.8`, `FLUTTER_CHANNEL=[user-branch]`, `FLUTTER_GIT_URL=unknown source`, `FLUTTER_FRAMEWORK_REVISION=c15ef63794`, `FLUTTER_ENGINE_REVISION=11e5695710`, `FLUTTER_DART_VERSION=3.12.2`. **Scope, stated honestly:** it bites where a program branches on one of those values (`const String.fromEnvironment('FLUTTER_VERSION')`), which is rarer than branching on a flavor, and **it is not demonstrated to break any app** — what is demonstrated is that the two kernels disagree. **Its own lane**, and the decision it needs is not just "forward them": whether they also belong in the FINGERPRINT. A release and a patch on one pinned cell would always agree on them, so the cost is low and the reasoning must still be written down. `evidence/g41-define-from-file/DECLARES.md`~~ — **SUPERSEDED by the row above, and its scope corrected there.** Kept struck through rather than deleted: a reader who acted on “it bites where a program branches on one of those values, which is rarer than branching on a flavor” needs to know that it applied to **every** iOS release, including one with no flavor and no defines at all |
| ◐ | **BUILT 2026-08-15 — CLOSED, and the row below is kept as its before-state.** Propagation and comparison are now two fields instead of one. `RouteBBuildConfig` gains **`injectedDefines`**, which feeds `compilerArgs` and **nothing else**: it is absent from `canonicalText`, `fingerprint` and `agreesWith` **by construction**, not by a filter someone has to remember — so release **95** and every release before it stays comparable to a patch cut today. **THE LEGACY RULE IS NARROW ON A MECHANISM ARGUMENT, not a kindness:** a replacement that reads the app's own `const` resolves it through the IMPORT KERNEL, which carries the real values since the row above; the ONLY expression that compiles against these `-D` flags is a `fromEnvironment` written in the replacement source ITSELF. So a pre-record release refuses **only** an environment-reading replacement, by name, and remains patchable for everything else — refusing all of them would have stranded releases 89–95 and every `killswitch_probe` permanently, to guard against a construct almost none of them contain. **`recordsInjectedDefines` separates *recorded none* from *predates the field*** — Flutter omits `FLUTTER_ENABLED_FEATURE_FLAGS` entirely when empty, so an empty recorded map is a FACT and not an absence, the same distinction `fromJson` already draws for the config as a whole. **THE PROBE NOW RUNS THROUGH THE PRODUCT'S OWN GETTER, and that is a vacuity fix worth naming:** `g41d`'s first draft hand-simulated the flags and asserted the value came out wrong — an arm that **would have kept passing after the defect was fixed**, because nothing connected it to the code under test. `probes/compiler_args.dart` now prints `RouteBBuildConfig.compilerArgs` itself, so the probe goes red if the product stops threading. **`g41d` 14/14** under a stamp guard that ABORTS on the wrong cell. **TWO NEGATIVE CONTROLS, each confirmed RED in its final form:** reverting `compilerArgs` to `effectiveDefines` alone fails 2, and disabling the legacy guard fails 1 — **with the two in-group controls GREEN in both states** (an ordinary replacement against a pre-record release still compiles; the same environment-reading replacement is ACCEPTED when the release did record), which is what stops the refusal test from meaning 'refuses everything'. **2513 passed / 1 skipped / 0 failed**; analyze `--fatal-warnings` exit 0, zero errors and zero warnings; format clean. **Host only — earns BUILT.** The device arm on `injected_define_app` is still owed and is what an upgrade needs. The before-state follows: **⚠ RETRACTION 2026-08-16 on the RELEASE half only.** A task-14 run recorded releases and patches on `cps-ios`; **the plane has no record of them** — verified three ways after a concurrent `G15` lane flagged it, and that lane was right. The compile-time results STAND and are backed by local artifacts: the release's `route_b.json` records all six `injectedDefines` with `effectiveDefines` `{}`, and the built replacement bytecode contains **`NEW-3.44.8`**. The **legacy REFUSAL also stands** — it happened locally with nothing uploaded. What is withdrawn is every *published* claim. **This row's status is unchanged at BUILT**, which is why the retraction costs nothing here: the release arm was never what earned it. See the new row below. |
| 🐞 | **KNOWN GAP, found 2026-08-16 — `cps-ios` reported success for operations it did not durably record.** A release and three patches were accepted, artifacts verified and a patch promoted, all with `200`/`204` in the container's own log; the CLI printed `✅ Published` for each and `releases list` showed the new release DURING the run. **The plane now has no record of any of it.** Verified three ways: `sqlite3` on the bind-mounted `/data/code_push.db` (max release **94**, 9 apps), live `GET /api/v1/apps` (the same 9), and `releases list` re-run (newest **87**). **Container never restarted** (`RestartCount 0`, `StartedAt 2026-08-13`), `DATA_DIR=/data` is the only `.db` it can see, the file has not been written since `2026-08-14T19:37` — the moment of G15's release 94 — and the `-wal` is 0 bytes. Disk has 50 GiB free and the container can still write to `/data`. **One clue not yet explained:** `GET /api/v1/apps/<new-app>/releases` returns **403, not 404**, so the server does not treat the app as absent. **CAUSE UNDETERMINED — deliberately not guessed at.** **Why this matters beyond one lane:** every release row in this file rests on the plane's word, and `R8`'s own prior row already noted the server records no engine/cell field. A plane that can report a durable success it did not persist makes *every* published claim provisional until re-read, and the re-read has to be against the DB or the API rather than the CLI's exit text. **Found by a concurrent lane's cross-check, not by this one** — the lane that cut the releases believed its own success output. **Its own lane.** `evidence/g41-injected-defines/G41D_DISCRIMINATING_ARM.md` **SHAPE SHARPENED 2026-08-16 by the `G15` lane, which confirmed the defect INDEPENDENTLY from the files rather than from this row, and deliberately did not duplicate the row.** Their characterisation is better than the one above and is adopted here with attribution: a **0-byte `-wal` with a CURRENT mtime beside a main DB that has not moved** means the database is being **opened, written through, and truncated with nothing committed into it**. So this is *not* a permissions wall and *not* a full disk — **the plane is doing the work and discarding it at commit**, which is exactly why it can answer `200`/`204` and print `Published` **in good faith**. It also explains the `403`: the route resolved and authorised against in-memory state that no longer exists on disk. **Cause still UNDETERMINED** — this is a sharper description of the shape, not a diagnosis. **THE REUSABLE RULE, which is worth more than the row it cost:** verify every publish by reading the **DB or the API afterwards**, never the CLI's exit text and never a same-session `releases list` — one client talking to one server is not an independent check. `G15` has written that into its own gate 5 rather than restating this row. **Boundary:** the last durable write is `G15`'s release **94** (`1.0.5+1`), which is the sharpest available timestamp on when this began; releases 89–94 and their artifacts ARE present, and `G15` confirmed its device measurements were taken from the STORED ARTIFACTS rather than from the plane's word. **⚠ THE MECHANISM STORY ABOVE IS RETRACTED 2026-08-16, INCLUDING THE PART THIS ROW ADOPTED FROM `G15`. The FINDING stands; the EXPLANATION was built on an invalid witness.** A controlled write was run against the live plane — one `POST /api/v1/apps`, then an immediate content read — and **it persisted**: the row is on disk and the server agrees, seconds later. So the plane is **NOT** discarding commits, and 'opened, written through, and truncated with nothing committed' is not supported. **WHY BOTH LANES GOT IT WRONG, and it is worth more than the wrong answer:** every mechanism claim rested on the DB file's **mtime and size**, and they did not move when a row was added — the probe write left `mtime=19:37:02 size=811008` **unchanged while demonstrably persisting**. **That pair is self-refuting on its own and needs no theory about the mount:** a row appeared and the size did not, so size never witnessed writes here. (An earlier draft of this row blamed Docker's bind mount for not propagating metadata. That is an unproven mechanism and is withdrawn — the observation stands without it, and the `G15` lane made the point first.) So 'the file has not been written since release 94' was never an observation about writes; it was an observation about a metadata channel that does not carry them here. The 0-byte `-wal` belongs to the same unreliable channel — and the INVERSE, which both lanes nearly missed: **a channel that can only ever say one thing will say it, so its silence is not a negative.** The 0-byte WAL was not evidence of no writes; it was evidence of nothing. The one-line check neither lane ran before reasoning from it: *what would a write LOOK like on this channel?* **WHAT SURVIVES, on CONTENT reads, which are reliable:** max release is **94**, max patch **60**, and the `injected-define-fixture` app is absent **under either organisation** — so the retraction of the publication claims is unaffected and was verified again here. **~~THE LEAD~~ — DEAD 2026-08-16, in both directions, and killed on CONTENT.** It was: the writes that vanished used a **user OAuth token** and the one that persisted used the **`API_KEY`**. `G15` produced the counter-example — releases **93/94** were cut through the ordinary user-OAuth CLI path and persisted — and rather than take it on trust this lane verified it in the `audit_log`, which settles it far better than either of us argued: **93/94 are actor `2` (`you@example.com`), releases 89–92 and patches 57–60 are actor `1` (`owner@self-host.local`), and the `API_KEY` probe is actor `1`. Both actors and both auth paths have persisted writes.** **AND THE AUDIT LOG GIVES THE REAL CHARACTERISATION, which neither lane had:** ids run **177 → 178 consecutively** across this lane's entire run — G15's release 94, then nothing, then the probe a day later. **My operations left no audit row at all.** So those transactions committed *nothing*, not even their own audit trail, while returning `200`/`204`. That is **not** a plane that discards commits (it does not) and **not** an auth-path difference (there is none). **THREE WITNESSES, each checked by the lane that did not claim it:** `G15` found `audit_log` **dense** (`max=178 count=178 min=1`), so no id was consumed and released — which distinguishes rollback from insert-then-delete; this lane extended the same test and **`releases` (94/94/1) and `patches` (60/60/1) are dense too**; and `sqlite_sequence`'s high-water marks **equal** `max(id)` on all three (`178/178`, `94/94`, `60/60`), so **no id was ever allocated beyond what survives.** **BUT THE EVIDENCE DOES NOT DISTINGUISH TWO READINGS, and calling it rollback picks one:** *(a)* the transactions reached this database and rolled back wholesale, or *(b)* **they never reached this database at all** — a server writing somewhere else entirely would leave exactly this trace. Those have different remedies and a vanish means different things under each, so the row said **undetermined between them**. **(b) IS NOW NEARLY DEAD, narrowed in two steps.** `G15` refuted the file half: `find` inside the container returns **exactly one** database, `/data/code_push.db`, so "the server wrote to a different DB file" is out — leaving only an in-memory database or a since-deleted one. This lane then closed most of what remained: **the same process** (`RestartCount 0`, `StartedAt 2026-08-13`, pid unchanged) **demonstrably wrote to `/data/code_push.db`** for the probe, so it was never configured for in-memory storage. For (b) to survive, one un-restarted process would have had to write somewhere else for four operations and then to its configured file for the fifth. **So (a) — reached this database and rolled back — is the leading reading**, and it is consistent with the sequence evidence, since SQLite's `AUTOINCREMENT` counter is transactional and a rollback reverts it. **Still not proven. THE OUTSTANDING ODDITY, RE-WORDED 2026-08-16 because the first wording was ambiguous and misled a reader:** it is **not** that a *rollback request* returned `200`/`204` — no rollback was requested, and the API is not implicated. The audit log shows **14 `patch.withdraw` `rollback=true` rows from 2026-08-04 to 2026-08-14, every one persisted**, so that path demonstrably works. **What is actually open:** four ordinary PUBLISH operations returned `200`/`204` while their transactions committed nothing — no release, no patch, no audit row, no sequence advance. **Established:** success statuses were returned for operations that left no durable trace. **NOT established:** that the server detected a rollback and reported success anyway, or any mechanism at all — an unhandled exception after the response is written, a transaction never opened, and a commit that silently failed would all look identical from outside. **Recorded against this lane:** the one-DB `find` had already been run HOURS earlier, while looking for the server's data path, and was not applied to (b) when (b) was proposed — evidence in hand and unused is its own failure, and it was the other lane that thought to point it at the hypothesis. **THE REMAINING CANDIDATE, and it points at THIS LANE'S OWN COMMITS, which is why it is written down rather than left as a shrug:** every persisted write was cut with rig CLI **`ba4e1c02`**; every vanished write was cut with **`50ed19a7`** or **`98f84c17`** — the re-syncs this lane performed. `50ed19a7` carries `G4.1c` (`72620b12`) but NOT link 2, so the new `injectedDefines` provenance field is exonerated; the CLI revision itself is not. **No mechanism is proposed** — the correlation is 6 persisted writes on the old CLI against 4 vanished on the new, which is a correlation and not a cause, and the honest reading is that this lane may have broken the publish path it then failed to verify. That is the clearest difference between the two, and it is NOT a diagnosis — the user-token path cannot currently be re-tested because `POST /token` now returns **400**, which is a separate and probably mundane refresh-token problem, since the `API_KEY` authenticates fine. **LEFT BEHIND:** the probe app `82ea958c…` `durability-probe-g41` is real state on `cps-ios`, created by this experiment and not cleaned up. **THE RULE THIS COST TWO LANES:** *a successful write is not evidence of durable state until persistence is independently observed* — and the observation must be of **CONTENT**, because file metadata is not a witness across a Docker bind mount. **STATUS CORRECTED 2026-08-16 — THE GENERAL DURABILITY BLOCKER IS CLEARED.** `cps-ios` can persist writes: the controlled write above landed and was confirmed by content read. **What remains is NARROWER and is what this row is now about:** the **user-OAuth write path** used by the vanished publishes is unexplained, while the `API_KEY` path is demonstrably durable. Publishing is therefore **not blocked in general** — it is blocked on the user-OAuth path until that is understood, and any publish must be verified by content read afterwards. **The re-login question is deliberately NOT answered here:** re-establishing user-OAuth mutates shared credentials, so it is worth doing only if a gate genuinely needs that path, and then as its OWN auth experiment with exclusive ownership of the credential and before/after content verification. **Credentials are not to be mutated merely to explain history.** **THE RESIDUE IS KEPT ON PURPOSE:** app `82ea958c…` `durability-probe-g41` stays as a positive control — a known write whose persistence was verified through content, and a cheap reference specimen. Delete it only after the auth-path question is resolved, **and record the deletion**. **THE AUDIT BOUNDARY IS UNAFFECTED:** release **94** stands, because the post-94 absence was established by CONTENT reads under both organisations — only the discarded-commit explanation was withdrawn, not the absence. **THE RULE, in its strict form, and it belongs beside the correction rule and `G15`'s reachable-failure rule rather than buried in this row:** *verify the property through a channel that can actually witness it — for persistence, read durable content or state, never host file metadata.* Left here rather than filed there because a concurrent lane is editing that section; the next doc pass should move it. |
| ☐ | ~~**KNOWN GAP, found 2026-08-15 by G4.1c's own discriminating fixture — the injected defines reach the KERNELS but not a PATCH BODY.** The row above threaded Flutter's six injected defines into the prepass and both import kernels and stopped there. **`route_b_producer.dart:169` feeds the replacement compiler `buildConfig.compilerArgs`, and `route_b_build_config.dart:345` builds those from `effectiveDefines` ALONE** — which the row above deliberately keeps the six out of, on the argument that a release and a patch on one pinned cell always agree on them so a fingerprint entry could only compare a constant with itself. **That argument is sound for COMPARISON and wrong for PROPAGATION:** the same field is also the source of the `-D` flags handed to the patch compiler. So a replacement body reading `const String.fromEnvironment('FLUTTER_VERSION')` **bakes the empty string while the release around it holds `3.44.8`** — the exact failure `route_b_producer.dart`'s own comment describes for user defines, *"a divergence no runtime check can see, because both are literals by then"*. **Measured, with the control that makes it a finding rather than a misattribution:** `probes/g41d_injected_define_patch.sh` **10/10** — arm 3 a replacement reading `FLUTTER_VERSION` does NOT get the value, arm 4 a replacement reading a USER define DOES (so the mechanism works and only this family is missing), arm 5 the same replacement DOES get it when the injected flags are passed (so the remedy is reachable). **Scope, honestly:** it bites only a patch body that itself reads one of the six, which is rarer than an ordinary patch, and **it is not demonstrated to break any shipped app** — what is demonstrated is that such a body compiles against the wrong value. **The fix is a DESIGN decision, not a mechanical change:** the six must reach `compilerArgs` **without** entering the fingerprint comparison, because releases already cut (including **95**) carry no such field and must stay comparable — separating PROPAGATION from COMPARISON is the shape of it. **Its own lane.** `evidence/g41-injected-defines/G41D_DISCRIMINATING_ARM.md`~~ — **SUPERSEDED by the row above, which closed it the same day it was opened.** Kept struck through rather than deleted: the reasoning it records — that excluding the six from the fingerprint also excluded them from the patch compiler — is why the fix is a SECOND FIELD rather than a wider fingerprint, and a reader who only saw the fix would not know why the obvious change was refused |
...
| ✅ | **CERTIFIED 2026-09-04 (CI-NONINTERACTIVE-1).** An automated job runs bootstrap → authenticate → release → patch → publish/promote with **fd 0 CLOSED** for every invocation, from the `selfhost-v1.1.1` bootstrap: release 10, patches 1 and 2, `beta`→`stable` via `set-track`, **0** interactive prompts on the certified path, the token in **0** captured logs and **0** audit rows, and 35 audit events each carrying a `request_id` and one credential FINGERPRINT. Closed stdin means `0<&-`, not `< /dev/null` — Dart reports a character device as a terminal, so under `/dev/null` `stdin.hasTerminal` is TRUE and the CLI believes it can prompt. The lane fixed one real defect: a **requested** confirmation (`--confirm`) was silently dropped when nothing could answer, so the mutation proceeded; it now refuses with exit 64, names the prompt, and does not mutate. Two prerequisites are non-obvious and are recorded in the contract — `GRADLE_USER_HOME` must be set because Gradle resolves `user.home` from **passwd** rather than `$HOME`, and the Flutter remote must carry upstream version tags or the SDK resolves `0.0.0-unknown`. One documented refusal remains by design: `set-track` cannot CREATE a channel unattended. Contract: `ci_noninteractive` in `engine/route_b/SUPPORTED_STATE.yaml`. Evidence: `evidence/ci-noninteractive-1/RESULT.md` | §10 |
...
**The shared-state fact that governs everything below:** the working tree at
`/Users/mendell/shorebird` is the only one anyone is *editing*. Both workers edit
the same files on the same branch. There is no isolation unless someone creates
it.

> **CORRECTED 2026-08-16 — this used to say `git worktree list` "returns exactly
> one entry", and it has not for some time.** It returns four: the shared tree
> plus `shorebird-compat-study`, `-study2` and `-study3`, all on detached HEADs
> at upstream commits, all clean, none touched since 2026-08-12. They are
> leftovers from the compatibility study, not lanes.
>
> The protocol below is unaffected — its rules are about the *shared* tree, which
> is still singular. But a reader checking the stated fact would have found it
> false and had no way to tell which half was stale, so **the invariant is
> restated as what it actually depends on: one tree being edited, not one tree
> existing.** Leftover worktrees are cheap to leave and cheap to remove
> (`git worktree remove`), and a clean detached one is not evidence of a lane.
...
|---|---|---|---|---|
| `R1` iPhone 7 | — | `G15` gate 5 — arm A only | released 2026-08-16 | **free. ARM A RAN AND IS INCONCLUSIVE; ARMS B AND C DID NOT RUN.** Left with `killswitch_probe` **`1.0.6+1`** installed (release 95, cell `cd137db6`), **patch 1 `Installed` and active**, `/Documents/g15_receipt` intact, `g15_armed` ABSENT. **Nothing uninstalled** — uninstalling resets iOS Local Network consent and blocks the app on a modal before any code runs. Launch with `idevicedebug run`, NOT `ios-deploy --justlaunch`. The next holder inherits a device already carrying an attached Route B patch whose value does not render; do not read that as a fresh baseline. Evidence: `evidence/g15/gate5_armA_inconclusive.txt`. Prior claim note: **HELD.** Running arms A/B/C against release **95** (`1.0.6+1`, cell `cd137db6`, carrying patch `0011`'s seam), outcomes precommitted in `evidence/g15/gate5_arms_precommit.md`. **[MUTATES the device]**: installs `1.0.6+1` and publishes patches against it. NOT uninstalling — that resets iOS Local Network consent and blocks the app on a modal before any code runs. Launch with `idevicedebug run`, NOT `ios-deploy --justlaunch`. Every arm reads `/Documents/g15_receipt` BEFORE the screen and pulls updater state BETWEEN launches, not only at the end. Prior release note: | **free. GATE 2 PASSES ON BOTH ENGINES.** Left with `killswitch_probe` **`1.0.5+1`** installed (cell `40eaa0ef`), `/Documents/g15_armed` ABSENT, `/Documents/g15_receipt` holding all 22 lines. `airgap_probe` untouched. Nothing uninstalled — uninstalling resets iOS Local Network consent and blocks the app on a modal before any code runs. Launch with `idevicedebug run`, NOT `ios-deploy --justlaunch`. Evidence: `evidence/g15/gate2_verdict.txt`. Prior claim note: **HELD.** Running gate 2 on the REPAIRED fixture (`1.0.4+1`), twice: first on cell `80e493e4` — the engine `1.0.3+1` failed on and `1.0.2+1` passed on, so it is the only engine where the failure can reproduce or be cured — then on `40eaa0ef`. **[MUTATES the device]**: installs `killswitch_probe` `1.0.4+1` over `1.0.3+1`. NOT uninstalling — that resets iOS Local Network consent and blocks the app on a modal before any code runs. Launch with `idevicedebug run`, NOT `ios-deploy --justlaunch`, which quits the app. Every arm reads `/Documents/g15_receipt` BEFORE the screen. Prior release note: **free. THE CONTROL RAN AND DID NOT CLOSE — read `evidence/g15/crashbackout_control_verdict.txt`.** `killswitch_probe` release `1.0.3+1` is **INERT**: three launches, blank white every time, and `/Documents/g15_armed` UNCHANGED across a launch, so `main()` is not executing in either arm. Updater state is `state.json` only — **no patch installed**, so the blank screen is not patch-caused and the crash-backout arm's attribution is retracted. Positive control on the same instrument PASSED (`airgap_probe` rendered fully, `control_positive_airgap.png`), so this is the fixture, not the rig, and not the instrument. Launch with `idevicedebug run`, NOT `ios-deploy --justlaunch`. Nothing uninstalled. Prior: **free.** Left with release `39.0.0+1` installed and patch 1 APPLIED (`code_patch=1`, screen reads `NEW-OBF`). NOT uninstalled — doing so resets iOS Local Network consent and the app then blocks on a modal before any code runs. Prior: **HELD.** Wired (`ioreg -p IOUSB` shows `iPhone@00140000`). Prior: **free.** Left with release `23.0.0+1` installed and patch 1 active (`code patch: 1`) but NOT applied — see the device-gate finding. The device is fine; the patch's kernel came from the wrong engine. Previously: Wired (`ioreg -p IOUSB` shows `iPhone@00140000`), iOS 15.8.8, online to `xctrace` as `8cb4bc98…`. Note `devicectl` reports it `unavailable` — iOS 15 is not a CoreDevice, so the install transport is chosen by version, not by that listing |
...
| `selfhost/fixtures/killswitch_app` (no `R-id`) | — | `G15` gates 1 and 2 | released 2026-08-14 19:40 | **free. GATES 1 AND 2 BOTH GREEN ON HARDWARE.** Both cuts done — `1.0.4+1` on cell `80e493e4` and `1.0.5+1` on `40eaa0ef` — and both pass. Version left at **`1.0.5+1`**; releases 1.0.0+1 … 1.0.5+1 exist on `cps-ios`, so bump before cutting another. `lib/main.dart` is in its RELEASE form (`bootProbe() => 'boot-ok'`, `routeBValue() => 'OLD-kill'`) so the next release cuts from it directly. `ios/` is generated and gitignored; regenerate with `prepare_killswitch_fixture.sh`, which now also injects the NATIVE half of the receipt into `AppDelegate.swift`. The receipt was rebuilt from a toggling bit into an APPEND-ONLY PHASE LOG at `/Documents/g15_receipt`, written by both halves of the app: `native launch` → `native engine` (injected into `AppDelegate.swift` by the prepare script, since `ios/` is generated) → `dart-main-entered` → `boot-probe-returned:X` → `arm:kill`/`arm:render` → `first-frame`. **The last line present names the point the launch reached**, so a gap localises the failure to one step — and the native half is the only thing that can separate *"Dart never started"* from *"the app never really launched"*, the two states the 2026-08-14 control conflated. Invariants, and they are load-bearing: the receipt write is the FIRST statement of `main()` ahead of every patchable call; `bootProbe()` is NOT wrapped in `try/catch` (the seam under test must see the unhandled throw, and guarding it would make the gate vacuous); an instrument fault renders red instead of killing. `flutter analyze` clean for `lib/main.dart` (the 25 remaining issues are the gitignored stock `test/widget_test.dart` and a patch-scratch file under `build/`); the injected Swift `swiftc -typecheck`s. **Version bumped to `1.0.4+1`** — releases 1.0.0+1 … 1.0.3+1 are published on `cps-ios`; bump again before cutting another. `lib/main.dart` is in its RELEASE form (`bootProbe() => 'boot-ok'`, `routeBValue() => 'OLD-kill'`) so the next release cuts from it directly. **NOT YET BUILT OR RELEASED — no device run has exercised the new receipt, so it is unproven on hardware.** **Two traps already paid for and fixed in the script:** `HOME` is unset on iOS (the marker path must come from `Directory.systemTemp.parent`), and `base_url` must be the Mac's LAN address because `localhost` on the device is the PHONE. **The device's updater state was WIPED** with `ios-deploy --rmtree` to clear a crash loop, so the app boots the release with no patch selected. Prior claim note: | **Claimed BEFORE the directory exists**, which is the whole lesson of the H2 double-take and of this session's own fan-out collision: a path with no `R-id` cannot be claimed unless someone writes the row. A fixture that kills its own process at a chosen moment during launch — deterministically standing in for swipe-away / launch watchdog / jetsam. G15's design records why `twoengine_app` CANNOT serve this arm: `_boot()` ends in `runApp()` and both entrypoints call it, so it is not headless and the prior design's arm 4 was a false green on that ground. |
| `~/.shorebird` rig state (no `R-id`) | — | `G15` gate 5 — the baseline cut | released 2026-08-16 | **free, AND THE STAMP IS LEFT AT `cd137db6…`, NOT RESTORED — read this before cutting anything.** The cache is warm and consuming the SEAM engine: `ios-release` `__TEXT,__text` `b3178e9e…`, dill `757d09d7…` with `_g15ReportLaunchSuccess` ×5, both verified on the CONSUMED artifacts. **Any release cut now carries patch `0011`'s seam.** That is deliberate — the next cycle needs it — but it is NOT the `40eaa0ef` PROVEN lineage, so a lane wanting that must restamp and re-warm (stamp, clear `bin/cache/{artifacts,dart-sdk,downloads}` and `*.stamp`, warm; **never write a stamp**, which asserts what the cache already holds). CLI at `98f84c17`, deliberately not reverted. Prior claim note: **HELD. [MUTATES shared rig state]** Switching the stamp to **`cd137db6…`** (the seam cell) by CLEARING `bin/cache/{artifacts,dart-sdk,downloads}` and `*.stamp` and forcing a real fetch — **never by writing a stamp**, which asserts what the cache already holds and would make the build consume the OLD engine while every report names the new one. Then warm and CONFIRM the `ios-release` Flutter binary EXISTS before cutting: `isRouteBEngine` returns false when it is merely absent, so the first release after a clear silently takes the non-Route-B path and still reports success. Then the ordinary user-OAuth baseline publish and the four-step protocol in `evidence/g15/gate5_baseline_precommit.md`. **Any release cut by another lane while this is held consumes the seam engine.** To restore: stamp `40eaa0ef…`, clear, re-warm. Prior release note: | **free. CLI is at `98f84c17`** (carries `f2982364`, the link-2 fix). **ENGINE STAMP UNMOVED across the whole run**, both measures: `engine.version` `40eaa0ef` and cached `ios-release` `__TEXT,__text` `bc0afffe`, before and after. **LIVE CAVEAT, now in BOTH directions:** releases cut from here record `injectedDefines`; releases **89–95** and `killswitch_probe` **1.0.2+1…1.0.5+1** do NOT, and cannot be given the field retroactively. A patch whose replacement reads `fromEnvironment` is REFUSED by name against those, while ordinary patches still work — measured, not predicted. **To restore an older CLI:** `git -C ~/.shorebird checkout <sha>`; the stamp keys on the git revision. Prior claim note: HELD. [MUTATES shared rig state]** Re-syncing the CLI from `50ed19a7` to HEAD so a release RECORDS `injectedDefines` — the field `f2982364` added. **The engine stamp is NOT touched**: `engine.version` stays `40eaa0ef`, guarded before and after. **Any release cut by another lane while this is held records the injected defines**, which releases 89–95 do not — that is the two-variable caveat the prior note describes, now moving in the other direction. Prior release note: free, AND THE CACHE IS RESTORED AND VERIFIED.** The cached `flutter_patched_sdk_product/platform_strong.dill` is back to the PUBLISHED `55e02ed8…`, confirmed by hash rather than assumed. Engine stamp untouched throughout: `engine.version` == `engine.stamp` == `40eaa0ef`, CLI `50ed19a7`. Prior claim note: **HELD. [MUTATES shared rig state]** Swapping `bin/cache/artifacts/engine/common/flutter_patched_sdk_product/platform_strong.dill` for `R3`'s, so a release build compiles `dart:ui` from OUR tree instead of `R4`'s. **The engine stamp is NOT touched — it stays `40eaa0ef`.** Any release cut by another lane while this is held compiles its app kernel against a dill that is not the published one, which no audit would catch, so treat this row as blocking for releases. **To restore:** delete that directory and `flutter precache --ios` through the mirror; the published zip refetches by stamp. Backup taken before the swap regardless. Prior release note: **free. RE-SYNC DONE — the CLI is now `50ed19a7`, and the debt G15 handed back is discharged.** Chosen over pinning `72620b12` because `git log 72620b12..HEAD -- packages/` is EMPTY; G15 independently confirmed the same, and further noted that `e149d0ca` and `82e455c8` touch **0 files** under `packages/`, so **every CLI behaviour change now in the snapshot is G4.1c’s** — the distinction that saves a future bisect. Rebuild verified rather than assumed: the snapshot contains `flutter_injected_defines` and the G4.1c warning string. **ENGINE STAMP UNMOVED ACROSS THE WHOLE RUN**, checked on both measures before and after release 40 + patch 1: `engine.version` `40eaa0ef…` and the cached `ios-release` `__TEXT,__text` digest `bc0afffe` (cell `40eaa0ef`; `136e3d64` would be `80e493e4`). **LIVE CAVEAT, and it cuts both ways:** every release cut from here carries the six injected defines in the prepass and both import kernels, while releases **89–95 and `killswitch_probe` 1.0.2+1 … 1.0.5+1 do NOT** — so any arm comparing a new release against those has **two variables and the CLI is the silent one**. G15 has already recorded that it will re-cut gate 4’s baseline rather than reuse `1.0.5+1`. **To restore `ba4e1c02`:** `git -C ~/.shorebird checkout ba4e1c02`; the next invocation rebuilds, since the stamp keys on the git revision. Prior claim note: HELD. [MUTATES shared rig state]** Taking the re-sync G15 explicitly handed back at `50ed19a7`. **Re-syncing the CLI from `ba4e1c02` to `50ed19a7`** — CLI-equivalent to `72620b12`, since `git log 72620b12..HEAD -- packages/` is EMPTY, so nothing in `shorebird_cli` changed after it. **The engine stamp is NOT touched**: `engine.version` stays `40eaa0ef6cb6485833bf2e10ac97224ca82cbf25`, and I verified G15’s restore independently before claiming rather than trusting the row — the cached `ios-release` `__TEXT,__text` digest reads `bc0afffe`, the `40eaa0ef` signature, not `136e3d64`. **Any release cut by another lane while this is held consumes a CLI carrying G4.1c’s injected-defines change**, which alters what the prepass and both import kernels are compiled with. Using G15’s recorded auth incantation (`AUTH_SERVICE_URL`, `SHOREBIRD_JWT_ISSUER`) rather than rediscovering it. **To restore:** `git -C ~/.shorebird checkout ba4e1c02` and let the next invocation rebuild the snapshot; the stamp is keyed on the git revision. Prior release note: free, AND THE STAMP IS RESTORED AND VERIFIED.** `engine.version` == `engine.stamp` == `40eaa0ef6cb6485833bf2e10ac97224ca82cbf25`, cache re-warmed, and the restore was CHECKED rather than assumed: the cached `ios-release` engine's `__TEXT,__text` reads `bc0afffe…` (cell `40eaa0ef`), not `136e3d64…` (cell `80e493e4`). Compare that way — an embedded/cached `Flutter` is re-signed and thinned, so file hashes never match the published xcframework. **THE CLI IS STILL AT `ba4e1c02` and the re-sync to `72620b12` is STILL OWED** by whoever cuts the next non-gate-2 release; it is explicitly handed back. **Newly documented, and it will stop the next person:** the vendored CLI cannot refresh a self-hosted credential — it POSTs to `https://auth.shorebird.dev/token` and 401s, which is why releases 89-92 worked (unexpired access token) and this session's did not. Cleared WITHOUT a re-sync by `AUTH_SERVICE_URL=http://10.0.0.7:18080` plus `SHOREBIRD_JWT_ISSUER=http://169.254.189.3:18080`; `ba4e1c02` already honours the former (`shorebird_env.dart:263`) with upstream only as a fallback. Full incantation in `evidence/g15/gate2_verdict.txt`. Prior claim note: **HELD. [MUTATES shared rig state]** Restamping `bin/internal/engine.version` to `80e493e4c5c9e4c418c433bf660392067a131dd5` and warming, cutting `killswitch_probe` `1.0.4+1`, then restamping to `40eaa0ef6cb6485833bf2e10ac97224ca82cbf25` and cutting `1.0.5+1`. **Any release cut by another lane while this is held silently consumes whichever engine is stamped at that moment** — no git conflict, no error. **The CLI is deliberately NOT re-synced**, even though `G4.1c` landed changes to `ios_releaser.dart` and `route_b_release_kernels.dart` and its row says the next release owes that step: gate 2 is a CONTROL against releases `1.0.2+1`/`1.0.3+1`, so holding the CLI constant is what keeps the fixture the only variable. Whoever cuts a non-gate-2 release next still owes the re-sync. **To restore:** stamp back to `40eaa0ef` and warm; both cells are published and served, so the revert is a stamp, not a rebuild. Prior release note: **free, AND THE STAMP IS RESTORED.** `engine.version` is back to `40eaa0ef6cb6485833bf2e10ac97224ca82cbf25` — the PROVEN lineage — so any release cut now consumes it rather than G15's cell. The cache still holds `80e493e4`'s artifacts; the next build for `40eaa0ef` refetches them by stamp, as this session demonstrated in both directions. To go back to G15's engine: stamp `80e493e4c5c9e4c418c433bf660392067a131dd5` and warm. Prior claim note: | **[MUTATES shared rig state]** Restamping `bin/internal/engine.version` in the pinned Flutter (`c15ef637…`) from `40eaa0ef` to the new cell **`80e493e4`**, clearing the artifact cache and re-warming it, so a release consumes G15's engine. **This is why the row exists: any release cut by another session while this is held will silently consume the G15 engine instead of the PROVEN `40eaa0ef` lineage** — no git conflict, no error, just a different engine underneath. `HANDOFF.md` records that re-syncing `~/.shorebird` invalidates another session's environment without touching git. **To restore:** stamp `engine.version` back to `40eaa0ef6cb6485833bf2e10ac97224ca82cbf25` and re-warm; both cells are published and served, so the revert is a stamp, not a rebuild. Checked before claiming: no uncommitted fixture version bump beside a fresh map line, i.e. §17's tell says no device gate was running elsewhere. |
| `R3` route-b tree | — | Route B — target→pool identity instrumentation | released 2026-08-16 | **free. BUILT AND COMPILE-VERIFIED; NOT PROVEN.** Captured as `engine/route_b/0012-routeb-target-pool-identity.patch`. The `0xd4a8`/`RouteBThing.value` special case is **DELETED**, and the pool location is now DERIVED from the target's identity by scanning the global pool for `entry.ptr() == target.ptr()`. **Multiplicity is preserved:** `TPOOL_ABSENT` (0 matches — a RESULT, since it is what an inlined callee looks like), `TPOOL_UNIQUE` (1, an identity), `TPOOL_AMBIGUOUS` (>1, first and second both reported). Offset via `ObjectPool::OffsetFromIndex`, the VM's own rule, never layout arithmetic. The supplied-offset path is demoted to an assertion (*does this slot hold the target?*); the six-state caller scan is untouched; trace bumped v4→v5 so a v4 reader cannot mistake missing `tpool_*` for a measurement. **THE PATCH SPANS TWO GIT CHECKOUTS** — `third_party/dart` is its own repo, so the flutter repo's `git diff` sees only `shorebird.cc`; the capture takes a second diff inside it, and the durable copies in `instrumentation/` are refreshed (`dart_route_b_trace.h` is UNTRACKED there, so that copy is the only record). **The negative control is OWED and named in the patch header**: feed the historical `0xd4a8` where it points at a valid but different object and require the assertion path to mismatch while the derived scan reports the truth. Until it exists, a green `tpool_status` is not evidence the demotion works. Compiles: `shorebird.shorebird.o` + `libdart_lib_aotruntime_product.object.o`, zero errors. No iOS build, no cell, no device. **Changes nothing about arm A.** Prior claim note: | **HELD. [MUTATES the engine tree]** Making the pool location DERIVED FROM TARGET IDENTITY instead of accepted as fixture-specific input. Five parts: delete the `0xd4a8`/`RouteBThing.value` special case outright; add a target→pool scan that PRESERVES MULTIPLICITY (0 = absent, 1 = authoritative index+offset, >1 = AMBIGUOUS reported, never silently first); demote the supplied-offset path to an independent assertion (*does this slot hold the target?*); leave the six-state CALLER scan untouched and unconflated; and add a negative control feeding the historical `0xd4a8` so the new path must refuse or mismatch rather than produce a confident identity. Two instruments, separated: the caller scan answers questions about a NAMED caller on request; the target-pool scan supplies the upstream measurement `--pool-offset` currently lacks. To be captured as `engine/route_b/0012-*.patch`. **Changes NOTHING about arm A** — `OLD-kill` stays unexplained and the target stays. Prior release note: | **free. `out/ios_release` REBUILT with patch `0011` — ninja exit 0, zero errors.** New engine `__text` `b3178e9e…`. **One fix the compile had NOT caught:** `gn --check` rejected the new `updater.h` include as not in any dependency of `//flutter/lib/ui:ui`. The host build had compiled that file happily, because adding an `#include` does not trigger a gn regen so the check never re-ran there — *"it compiled"* was true and was never *"the dependency graph permits it"*. Fixed by adding `//flutter/shell/common/shorebird:updater` to `lib/ui`'s deps (verified leaf, no cycle) and **recaptured into `0011`, now seven files**. The nodm dill was rebuilt with the seam too (`757d09d7…`, `_g15ReportLaunchSuccess` ×5) and postdates the last `hooks.dart` edit — checked, because a stale dill would ship an engine with the FFI registered and no Dart caller, banking nothing and tombstoning every patch after two boots. Prior claim note: | **HELD.** Rebuilding `out/ios_release` so patch `0011`'s seam exists in a shippable engine, ahead of a mint from `87130ae8`'s corrected publication path. **No source edit** — `0011` is already in the tree and captured; this is a compile only. **[MUTATES `out/ios_release`]**, which is the cell-`80e493e4` engine, so the next mint's `ios_artifacts_sha256` will differ by construction and a NEW cell address follows — existing cells are untouched. Nothing published, no device. Prior release note: **free. THE SEAM IS BUILT AND COMPILES; IT IS NOT PROVEN.** Captured as `engine/route_b/0011-g15-runmain-seam.patch` — six files: `hooks.dart` (wrap `userMainFunction(args)`, capture the discarded `Object?`, catch→report→**RETHROW**, observe the returned `Future`, latch first `_drawFrame`), `platform_configuration.{h,cc}` + `dart_ui.cc` (two FFI statics forwarding to `Updater`, deliberately the thinnest shim so no second opinion about launch state can exist), `shell.cc` (**success report DELETED**), `shell_unittests.cc` (two tests corrected). **`0011` SUBSUMES `0009`'s `shell.cc` hunk — do not apply both**; it was captured against the pinned base while `0009` was live, so that portion contains both edits, and the header says so. **Two stale comments fixed rather than left to mislead:** the constructor's *"Success now fires in Shell::RunEngine"* is now false and is marked superseded in place. **Compile status: `dart:ui` builds (2/2), `ui.dart_ui.o` + `ui.platform_configuration.o` build (598/598, zero errors), `common.shell.o` builds.** The shell unittest BINARY was not built, so the two corrected tests are unrun. No iOS engine build, no cell, no device. Tree health on release: `dart_patches.sh --verify` → `OK: all 5 patches applied on the pinned base`. Prior claim note: **HELD. [MUTATES the engine tree]** Implementing the DECIDED policy `success = earliest(main completion, first framework frame)` — queue rows 4b/4c. Four files: `lib/ui/hooks.dart` (wrap `userMainFunction(args)`, capture the `Object?`, catch→report→**RETHROW**, observe the returned `Future`, plus a first-invocation latch in `_drawFrame`), `lib/ui/window/platform_configuration.{h,cc}` (statics on `PlatformConfigurationNativeApi` calling `shorebird::Updater::Instance()`), `lib/ui/dart_ui.cc` (`FFI_FUNCTION_LIST`). **`shell.cc`'s patch-`0009` SUCCESS call MUST GO, and it is not optional:** `Engine::Run` returns before any user Dart, so its `ReportLaunchSuccess` takes the SHARED latch and would make the new seam permanently unreachable. **`0009`'s FAILURE call STAYS** — a `Run` returning `Failure` means Dart never ran, which the Dart seam cannot observe, and the two are mutually exclusive so they cannot race. Invariant carried from the decision: **both reporters feed the EXISTING atomic latch; no new lifecycle interpretation at `Shell`/raster level.** To be captured as `engine/route_b/0011-*.patch` — `R3` is not in git and an uncaptured edit evaporates when the tree is recreated. Prior release note: **free. GATE 3's POSITIVE HALF PASSES, and the probe did NOT outlive it.** A marker in `_runMain`, retained by construction (`if (args.length == 0x7ffffffe) throw`, unfoldable because `args` is runtime data), reached a built release's `App.framework` — and was ABSENT from the same app built against the published dill. Negative control, so not vacuous. **Everything restored and CHECKED:** `lib/ui/hooks.dart` reverted (0 markers, clean against HEAD); `out/host_release_arm64_nodm`'s dill rebuilt back to `9f5a5f75…` byte-for-byte with `attach` ×8 and 0 markers, which incidentally shows the platform-dill build is deterministic; `out/host_release_arm64` never touched. Free bonus: R3's dill AOTs cleanly against `40eaa0ef`'s cached toolchain (exit 0, no "Unexpected tag 4"), so the dill can move WITHOUT the dart-sdk following it — one measured pairing, not a general licence. Evidence: `evidence/g15/gate3_positive_verdict.txt`. Prior claim note: **HELD. [MUTATES the engine tree]** Placing a uniquely greppable marker in `lib/ui/hooks.dart`'s `_runMain` and rebuilding `flutter_patched_sdk_product`, to answer gate 3's positive question — *does a `hooks.dart` edit reach a built release's AOT when the RIGHT dill is consumed* — **without a mint**. Cheapest-refutation-first: if the marker does not land even with the dill fed in directly, the `_runMain` candidate is dead for a second, independent reason and the publication repair would have been wasted. The marker must be **retained by construction** (a throw on a branch the compiler cannot fold), or its absence cannot be told from tree-shaking — a vacuous check in exactly the shape this process forbids. The edit is a PROBE and will be reverted or captured explicitly, not left in the tree. Prior release note: **free.** `out/host_release_arm64_nodm` BUILT (1458/1458 for the platform-dill target). **One source edit, and it is CAPTURED:** `runtime/lib/object.cc`'s `RouteBFunctionField` is now guarded by `#if defined(DART_DYNAMIC_MODULES)` — unguarded, any `dm=false` build dies on `-Werror,-Wunused-function`, so the tree had only ever compiled in one of its two configurations. Captured to `engine/route_b/instrumentation/{lib_object.cc.snapshot,dart-lib-object.fulldiff.patch}`, which is where this file's changes live (`object.cc` is NOT in `dart_patches.sh`'s `PATCHES`, so `--verify` does not cover it). **`out/host_release_arm64` was NOT touched** — its dill is the cell-identity input. **Result that changes the plan: the platform dill is dm-INVARIANT in R3** (dm, nodm and ios_release all yield `9f5a5f75…`, `attachBytecodeToFunction` ×8), so the publication fix needs no new build at all — R3's existing zip is already the dill every cell address was computed over. **Trap left behind:** the nodm `dart-sdk-darwin-arm64.zip` step FAILED (malformed `//`-rooted path to `dart_sdk_entitlements.txt`) and left a 138 MB PARTIAL zip with a plausible size in `out/host_release_arm64_nodm/zip_archives/`. **Do not publish it.** Nothing minted, nothing published, overlay byte-unchanged. Evidence: `evidence/g15/gate3a_platform_dill_repair.txt`. Prior claim note: **HELD.** Building a NEW out dir `out/host_release_arm64_nodm` so the platform dill can be published from the tree that owns the iOS engine and the Route B Dart patches. **Config derived, not guessed:** `out/host_release_arm64/args.gn` minus its `dart_dynamic_modules = true` line, which is exactly how `R4`'s nodm dir differs from its dm one. **Why nodm is right here even though `R3`'s `ios_release` is `dm=true`:** the "Unexpected tag 4 (Field)" coupling is between the platform dill and the FRONTEND_SERVER, and the published frontend_server (`publish_ios_overlay.sh:213`, `R4 host_debug_arm64`) is `dm=false`; `gen_snapshot` being `dm=true` is the combination that already builds releases today. **`R3`'s existing `out/host_release_arm64` is NOT touched** — its dill (`9f5a5f75…`) is the cell-identity input `FLUTTER_PLATFORM` names, and clobbering it would rewrite the meaning of every existing address. **[MUTATES the engine tree]** only by adding an out dir; no source edit, so nothing new to capture as a patch. Prior release note: **free. TREE HEALTH ON RELEASE: GREEN** — `dart_patches.sh --verify` → `OK: all 5 patches applied on the pinned base`, run at release. The tree carries TWO NEW uncommitted edits, and both are CAPTURED: `shell.cc` (patch `0009`) and `third_party/updater`'s `lifecycle.rs` (patch `0010`). **Note for the next holder:** `third_party/updater` is its OWN git checkout, so the engine repo's `git diff` does NOT see the Rust change — capturing it needed a second `git diff` inside that directory, and missing that would lose half the work silently. `out/ios_release` was rebuilt and consumed into cell `80e493e4`. Prior claim note: **TREE HEALTH AT CLAIM: GREEN** — `dart_patches.sh --verify --dest .../third_party/dart` → `OK: all 5 patches applied on the pinned base` (0001, 0004, 0005, 0006, 0008), run immediately before claiming. Implementing `plans/G15-crash-backout-and-restart-required.md`: moving the launch-success report out of the `Shell` CONSTRUCTOR (`shell.cc:537`, gated only on `vm_` existing, so it banks success before the root isolate exists) to after `Engine::Run` returns — the earliest point that proves patched Dart actually started. Also claiming **`R11`** (the mint this needs) and **`R1`** (wired, `iPhone@00140000`) for the device arms. **[MUTATES the engine tree]** — the change will be captured as `selfhost/engine/route_b/0009-*.patch`, because `R3` is not in git and an uncaptured edit evaporates when the tree is recreated. Prior release note: **free. Tree health on release: GREEN** — `dart_patches.sh --verify` → `OK: all 5 patches applied on the pinned base`, working tree clean. `out/ios_release` rebuilt from patch `0008` (ninja exit 0, `assert_mint_ready.sh` → `VERDICT=success`) and consumed into cell `40eaa0ef`. Prior claim note: **HELD. Tree health at claim: GREEN** — `dart_patches.sh --verify` → `OK: all 5 patches applied on the pinned base`, working tree clean, Dart base `6b58bb3a` over `d684a576`. Building `out/ios_release`; the host tier is already committed at `4bcdcb9b`. Prior release note follows: **free. Tree health on release: GREEN** — `dart_patches.sh --verify` → `OK: all 5 patches applied on the pinned base`. The Dart subtree carries a NEW edit, and it is captured: `selfhost/engine/0008-dart-load-obfuscation-map.patch` (`--load-obfuscation-map`), appended to `PATCHES`. `out/host_release_arm64/gen_snapshot` was rebuilt from it (ninja exit 0) and advertises the flag; `out/ios_release` was **not** rebuilt, so no mint and cell `4df8f9b6` is untouched. **Note for the next holder:** `0008` is the first Dart patch to touch a file another patch already touches (`0005` also edits `precompiler.cc`), so `PATCHES` order is now load-bearing rather than conventional — and an insertion adjacent to `0005`'s hunk context makes `0005` report `[CONFLICT]`, which is what happened once here and was fixed by relocating the added flags clear of it. Previously: released 2026-08-12, `dart_patches.sh --verify` all 4 applied, `route_b_analyze.aot` + `route_b_gen_dynamic_interface.aot` rebuilt and now published in cell `ee001fd7`. Nothing uncommitted in the Dart subtree; `$OUT/zip_archives` holds the exact bytes that cell carries |
| `R4` ios-engine tree | — | — | — | **free** |
| `R6` canonical fixture | — | `G4.1c` — the LEGACY specimen | released 2026-08-15 | **⚠ RETRACTION 2026-08-16 — THE PUBLICATION CLAIMS IN THIS ROW ARE WITHDRAWN.** A concurrent `G15` lane could not find the releases and checked before saying so; I verified and **they are right**. The control plane now reports `airgap-fixture`'s newest release as **87 / `39.0.0+1`** — there is no release 95, no `40.0.0+1`, no patch 2, and no `injected-define-fixture` app. Confirmed three ways: `sqlite3` on the container's bind-mounted `/data/code_push.db` (max release **94**, 9 apps, mine absent), the live `GET /api/v1/apps` (same 9), and `shorebird releases list` re-run (newest 87). **The operations DID happen** — the container's own log shows `POST .../patches -> 200`, `artifact verified`, `promote -> 204` against app `edd7188c…` — and the CLI reported success at the time, and `releases list` showed 95 during the run. **They were not durably recorded.** The container never restarted (`RestartCount 0`), `DATA_DIR=/data` and that is the only `.db` it can see, yet the file has not been written since `2026-08-14T19:37` — G15's release 94 — and the `-wal` is 0 bytes. **Cause UNDETERMINED and not guessed at here.** **WHAT SURVIVES, because it is local and does not depend on the plane:** the release's own `route_b.json` (committed at `evidence/g41-injected-defines/release_1_route_b.json`) records all six `injectedDefines` with `effectiveDefines` `{}`, and the built `replacement_0.bytecode` still contains **`NEW-3.44.8`**. Those are COMPILE-TIME facts about artifacts on this disk and are unaffected. **WHAT IS WITHDRAWN:** every "published" and "confirmed server-side" claim. **The refusal arm is unaffected** — it refused locally, before any upload, and nothing was uploaded by design. **The legacy REFUSAL stands; the "patch 2 published" control does not.** ORIGINAL ROW FOLLOWS, retained so the retraction is legible: **free at `40.0.0+1`, `lib/main.dart` restored to its RELEASE form** (`value()` returns `OLD-rel`), verified by `git status`. **No new release cut.** Used as the LEGACY specimen against release 95, twice: an env-reading replacement was **REFUSED by name with nothing uploaded**, and an ordinary one **published as patch 2** — the control proving old releases are not globally unpatchable. Prior claim note: HELD.** Used for the legacy arm ONLY, and **no new release is cut from it**: release **95** (`40.0.0+1`) is already a genuine pre-field specimen, because `route_b_build_config.dart` changed only in `f2982364` and 95 was cut with `50ed19a7`. `lib/main.dart` takes a TRANSIENT patch-form edit reading `String.fromEnvironment`, which must be refused, and is restored immediately after. Version is NOT bumped. Prior release note: free at `40.0.0+1`.** `lib/main.dart` is back in its RELEASE form (`value()` returns `OLD-rel`) — verified by `git diff` showing **pubspec.yaml as the only changed file**, so release 41 can be cut from it directly. The patch form used for release 40’s arm was `NEW-G41C`. **EVIDENCE CEILING HELD:** this was an INTEGRATION REGRESSION arm and is recorded as one. `airgap_app` reads none of the six injected defines, so its kernels are byte-identical with and without them and this release **cannot** and **does not** claim the G4.1c path works — only that the corrected CLI still traverses release → patch. `evidence/g41-injected-defines/RELEASE_40_INTEGRATION_ARM.md`. Prior claim note: HELD.** Cutting release **40** from `airgap_app` at `40.0.0+1`, **UNMODIFIED** — `lib/main.dart` is left in its RELEASE form and only `pubspec.yaml`’s version moves. **EVIDENCE CEILING, fixed before the run rather than after it:** this is an INTEGRATION REGRESSION arm ONLY. It may claim that the G4.1c CLI still traverses the real iOS release → patch path, and NOTHING about injected defines — `airgap_app` reads none of the six, so its prepass and import kernels are byte-identical with and without them, and a green release cannot observe the fix. The discriminating evidence is banked host-side at `probes/g41c_injected_defines.sh`. Any upgrade from BUILT needs a fixture whose REACHABLE program consumes one of the six observably. Prior release note: free at `39.0.0+1`.** `lib/main.dart` is in its RELEASE form (`value()` returns `OLD-rel`) — restored 2026-08-14 after G4.3's patch arm, so release 40 can be cut from it directly. The patch form used for release 39's arm was `'NEW-OBF'`; `dart analyze` clean after the restore. Prior: **HELD.** `airgap_app` currently `38.0.0+1`. Prior note stale on version: **free at version `23.0.0+1`.** `lib/main.dart` is left in its PATCH state (`value() => _secret`), not the release state — whoever resumes either reverts that line or treats it as the next patch's source. Previously: Added one private field the release never reads — being unread is the condition under test, since retention then depends entirely on P2's `--private-dill` enumeration. Version bumps to 23 with this release |
...
| `R8` `cps-ios` | — | `G4.1c` — task 14, both specimens | released 2026-08-15 | **⚠ RETRACTION 2026-08-16 — THE PUBLICATION CLAIMS IN THIS ROW ARE WITHDRAWN.** A concurrent `G15` lane could not find the releases and checked before saying so; I verified and **they are right**. The control plane now reports `airgap-fixture`'s newest release as **87 / `39.0.0+1`** — there is no release 95, no `40.0.0+1`, no patch 2, and no `injected-define-fixture` app. Confirmed three ways: `sqlite3` on the container's bind-mounted `/data/code_push.db` (max release **94**, 9 apps, mine absent), the live `GET /api/v1/apps` (same 9), and `shorebird releases list` re-run (newest 87). **The operations DID happen** — the container's own log shows `POST .../patches -> 200`, `artifact verified`, `promote -> 204` against app `edd7188c…` — and the CLI reported success at the time, and `releases list` showed 95 during the run. **They were not durably recorded.** The container never restarted (`RestartCount 0`), `DATA_DIR=/data` and that is the only `.db` it can see, yet the file has not been written since `2026-08-14T19:37` — G15's release 94 — and the `-wal` is 0 bytes. **Cause UNDETERMINED and not guessed at here.** **WHAT SURVIVES, because it is local and does not depend on the plane:** the release's own `route_b.json` (committed at `evidence/g41-injected-defines/release_1_route_b.json`) records all six `injectedDefines` with `effectiveDefines` `{}`, and the built `replacement_0.bytecode` still contains **`NEW-3.44.8`**. Those are COMPILE-TIME facts about artifacts on this disk and are unaffected. **WHAT IS WITHDRAWN:** every "published" and "confirmed server-side" claim. **The refusal arm is unaffected** — it refused locally, before any upload, and nothing was uploaded by design. **This row is the one that most needs the correction: it asserted a measured result on releases that the plane has no record of.** ORIGINAL ROW FOLLOWS, retained so the retraction is legible: **free. BOTH TASK-14 SPECIMENS RUN.** New app `injected-define-fixture` `edd7188c-ea3c-d585-8c87-02cb7b563e2a`, release **`1.0.0+1`** on cell `40eaa0ef` with all five Route B artifacts, and **patch 1 published**. **The release records `injectedDefines` (all six) while `effectiveDefines` stays `{}`** — the propagation/comparison split visible in a real artifact — and **the published replacement baked `NEW-3.44.8`, not `NEW-`**, which is the defect `g41d` arm 3 found, measured on a container that was actually uploaded. Release 95 additionally carries **patch 2** from the legacy control. No device. Check 4b still reports its one finding on this cell. Prior claim note: HELD.** Container healthy. Registering ONE app for `injected_define_app` and cutting ONE release plus a patch (the POSITIVE specimen), and building — **not publishing** — a patch against the existing release **95** (the LEGACY specimen, which must REFUSE). No device. Prior release note: free. RELEASE 95 (`40.0.0+1`, ios active) AND PATCH 1 PUBLISHED**, both first try, on cell `40eaa0ef` — confirmed server-side with `releases list`, not merely from the CLI’s success text. Patch: stable, arm64, 797 B. **7156 patchable call sites** (1778.4/MiB); all five Route B artifacts captured including `release_import.dill`, whose presence means `agreesWith` PASSED with the injected defines threaded. `buildConfig.effectiveDefines` is `{}` — **correct and designed**, since the fixture supplies no `--dart-define` and the injected six are deliberately outside the fingerprint; read as "the defines never arrived" it would be the wrong artifact. **CHECK 4b REPORTED ITS ONE FINDING AND IT IS RECORDED, NOT SUPPRESSED:** the cell’s address is computed over `9f5a5f75…` while builds download `55e02ed8…`, so release 40 carries a known-open provenance defect (G15’s `82e455c8`). Container healthy; no device touched. **Two traps worth inheriting:** the patch’s App Store IPA export fails (`No Accounts`) because the release was cut `--export-method development` — not a patch failure; and `shorebird patch` rebuilds `build/`, deleting the release’s `route_b.json`, so capture it BEFORE patching. Prior claim note: HELD.** Container up (healthy, 28h+). Publishing ONE release (`airgap_app` `40.0.0+1`, app `5c405fd6-3c34-2cb7-86e0-1291b9af666c`) and ONE matching patch against it, as the integration regression arm for `G4.1c`. No device. **Check 4b will report exactly one finding** on cell `40eaa0ef` — the delivered `platform_strong.dill` (`55e02ed8`) differs from the one the cell address was computed over (`9f5a5f75`), G15’s `82e455c8`. That is the check working; it will be RECORDED as a known-open provenance defect, never suppressed and never routed around. Prior release note: free. RUN COMPLETE: §4's real-third-party-app gap is CLOSED on the build clause.** App `589036b4-39ee-389b-c9fe-94fd42474a03` registered, release **88** (`2.2.7+236`) published, iOS active, cell `40eaa0ef`. Container left up and healthy; no patch published, no device touched, and the app is left registered so a patch arm can ride this release without a re-cut. **One provenance gap found and NOT fixed here:** the control plane's release record carries `flutter_revision` but **no engine or cell field**, so the server cannot answer "which Route B cell produced this release" — only the shipped `route_b.json` can. The authoritative server-side record is weaker than the on-device artifact, which is worth closing before anyone audits a release from the console alone. Prior claim note: **Container up (healthy).** Registering ONE app and cutting ONE Route B iOS release of the real third-party app at `/Users/mendell/compat-corpus/wonderous` — the run §4's KNOWN GAP row says is owed. No patch, no device. Also claiming that corpus path (no `R-id`, so it could not otherwise be claimed — the H2 lesson). Reading `~/.shorebird` only; **not** resyncing it, because the remedy under test (`cd453304`, import-kernel private enumeration) is already in the rig CLI at `ba4e1c02` and resyncing would mutate another session's environment for no gain. The rig CLI therefore LACKS this branch's later commits — `f06fa056`'s flavor-spelling fix among them — which is acceptable and worth stating: Wonderous is unflavored, so that call site resolves identically. Prior: **free.** Release 39 (`39.0.0+1`) and patch 1 published against it. Prior: **HELD.** Container up (healthy). Prior: **free.** Release 69 (`23.0.0+1`) and patch 1 published against it. Previously: Container up and healthy; one release and one patch will be published against it |
...
| `packages/shorebird_cli` — `lib/src/flutter_injected_defines.dart` (new), `lib/src/route_b_release_kernels.dart`, `lib/src/commands/release/ios_releaser.dart` + their tests; `selfhost/engine/route_b/probes/g41c_injected_defines.sh`; `selfhost/evidence/g41-injected-defines/`; `PARITY.md` §4 injected-defines row (no `R-id`; **`R7` is NOT taken** — nothing here edits `route_b_producer.dart` or `analyze_coverage.dart`, so `analysisVersion` stays 8) | — | `G4.1c` — Route B analyzes the program Flutter ships | released 2026-08-14 | **free. TREE HEALTH ON RELEASE: GREEN.** Suite **2504 passed / 1 skipped / 0 failed**; `dart analyze --fatal-warnings lib test` **exit 0**, zero errors and zero warnings; `dart format --set-exit-if-changed .` 358 files, 0 changed. Probe `g41c_injected_defines.sh` **5/5** against cell `40eaa0ef` and pinned Flutter `c15ef637`, on a throwaway `flutter create` app — **no committed fixture was read or written**, so `flavored_app` and `killswitch_app` were untouched. Read-only use of `R11` (the mirror) for `engine_stamp.json`; **`~/.shorebird` was NOT re-synced**, and the next lane to cut a release with this CLI owes that step. **Staged through a SEPARATE TEMPORARY INDEX** (`GIT_INDEX_FILE`), because a concurrent `G15` lane had its killswitch-fixture work and one claims-row line **staged in the shared index** while this committed — so neither `git add` nor a plain `git commit` was safe, and §17’s patch-file route could not separate them either once they were in the index rather than the working tree. That lane’s staged work was left staged and untouched; **verified in both directions** after `e7e13b7c` landed mid-work. **No release was cut, so this earns BUILT and not PROVEN.** |
| `selfhost/fixtures/injected_define_app` (no `R-id`), `selfhost/engine/route_b/probes/g41d_injected_define_patch.sh`, `selfhost/evidence/g41-injected-defines/` | — | `G4.1c` — task 14 release/patch arm | released 2026-08-15 | **⚠ RETRACTION 2026-08-16 — THE PUBLICATION CLAIMS IN THIS ROW ARE WITHDRAWN.** A concurrent `G15` lane could not find the releases and checked before saying so; I verified and **they are right**. The control plane now reports `airgap-fixture`'s newest release as **87 / `39.0.0+1`** — there is no release 95, no `40.0.0+1`, no patch 2, and no `injected-define-fixture` app. Confirmed three ways: `sqlite3` on the container's bind-mounted `/data/code_push.db` (max release **94**, 9 apps, mine absent), the live `GET /api/v1/apps` (same 9), and `shorebird releases list` re-run (newest 87). **The operations DID happen** — the container's own log shows `POST .../patches -> 200`, `artifact verified`, `promote -> 204` against app `edd7188c…` — and the CLI reported success at the time, and `releases list` showed 95 during the run. **They were not durably recorded.** The container never restarted (`RestartCount 0`), `DATA_DIR=/data` and that is the only `.db` it can see, yet the file has not been written since `2026-08-14T19:37` — G15's release 94 — and the `-wal` is 0 bytes. **Cause UNDETERMINED and not guessed at here.** **WHAT SURVIVES, because it is local and does not depend on the plane:** the release's own `route_b.json` (committed at `evidence/g41-injected-defines/release_1_route_b.json`) records all six `injectedDefines` with `effectiveDefines` `{}`, and the built `replacement_0.bytecode` still contains **`NEW-3.44.8`**. Those are COMPILE-TIME facts about artifacts on this disk and are unaffected. **WHAT IS WITHDRAWN:** every "published" and "confirmed server-side" claim. **The refusal arm is unaffected** — it refused locally, before any upload, and nothing was uploaded by design. **No release exists from this fixture.** ORIGINAL ROW FOLLOWS, retained so the retraction is legible: **free at `1.0.0+1`.** First release cut from it; `lib/main.dart` restored to its RELEASE form. The patch form used was `replacementReadsDefine` reading `FLUTTER_VERSION`. **`pubspec.lock` is now generated and untracked** — left uncommitted deliberately, since the fixture pins nothing and the lockfile is reproducible from `flutter pub get`. Prior claim note: HELD.** Cutting the first release from this fixture at `1.0.0+1`. `lib/main.dart` takes a transient patch-form edit and is restored after. Prior release note: free. FIXTURE BUILT AND ITS HOST PROOF IS 10/10 — and it found a second defect.** Claimed BEFORE the directory existed, per the H2 lesson: a path with no `R-id` cannot be claimed unless someone writes the row. `flutter analyze lib` clean; `lib/main.dart` is in its RELEASE form and the patch forms are tabulated in the fixture README. **NO release cut, NO device run, and `~/.shorebird` was READ ONLY** — the probe copies the fixture and runs `flutter create`/`pub get`/`--config-only` into the COPY, never the committed tree, under a stamp guard that ABORTS if `engine.version` is not the expected cell. Guard held: `40eaa0ef…` before and after. **Link 1 (analysis) is fixed and the kernels are BYTE-IDENTICAL; link 2 (patch replacement) is a new KNOWN GAP** in §4. **Two instrument findings recorded so nobody rebuilds an arm on them:** `gen_kernel --aot` does not tree-shake, and `route_b_analyze`'s `changed` does not see a constant-only body difference — an earlier draft of the probe passed for the wrong reason because of the second. **Deliberately NOT touched, per the lane's scope:** `publish_ios_overlay.sh`, the check-4b repair, CLI provenance, and cross-release metadata. Those are system-level concerns and are not this fixture's question. |
| `packages/shorebird_cli` — `lib/src/archive_analysis/archive_differ.dart`, `lib/src/route_b_release_kernels.dart` + their tests (no `R-id`; **`R7` is NOT taken** — neither file is the producer or the coverage analyzer, so `analysisVersion` stays 8) | — | the three Windows CI failures — TWO independent claims | released 2026-08-16 | **free. BOTH CLAIMS RETIRED — CI-PROVEN on the environment that could observe them.** Fork run `31989293325`: `Build shorebird_cli (windows-latest)` → **success**, alongside macOS and ubuntu. Per the precommitted rule, Windows green retires both: `09166a9b` (handle leak, closed in `finally`) and `4881aa97` (host-canonical entrypoint path). The three failures are gone and the two new regressions passed on the only platform where they can fail. Prior state: **BOTH FIXED; CI VERIFICATION PENDING.** Worktree used and removed; both cherry-picked. Gates on release: format 0, `analyze --fatal-warnings` 0, suite **2516 passed / 2 skipped**. **NEITHER FIX IS PROVEN BY A LOCAL RED/GREEN** — that is the honest limit and it is inherent, not laziness: both regressions are vacuously green on POSIX. Claim 2 was reproduced without Windows using the path package's own windows context, matching the CI log's Actual and Expected strings exactly; claim 1 has no such substitute and awaits the Windows leg. **What the next run decides:** Windows green retires both; Windows still red on either test means that fix is refuted. Prior claim note: **CLAIMED BEFORE THE FIX. In a WORKTREE per §17 rule 3** — `packages/` code, no overlay dependency, so no deviation this time. **NONE of the three is a flake or a test artifact; both causes are production defects that POSIX hides.** **(1) HANDLE LEAK:** `archive_differ.dart:148` does `ZipDecoder().decodeStream(InputFileStream(archivePath))` and never closes the stream; same in `fileHashes`. POSIX permits deleting an open file, so macOS/Linux never notice — Windows returns `errno 32` and the `tearDown` deletion fails. Real on every platform; **Windows is the only environment that can observe it**, and the regression will say so rather than manufacture a fake to make POSIX pretend. **(2) PATH CANONICALIZATION:** `p.join(root, 'lib/main.dart')` on Windows yields `…\lib/main.dart` — joined with `\`, embedded `/` untouched. The input shape is legitimate (it is what `flutter build` supplies), so production owns canonicalization: `p.normalize(p.join(…))`. The test is NOT relaxed — comparing normalized forms in the test would bless mixed-separator output and hide the gap |
| `selfhost/evidence/mint-provenance/` (no `R-id`; **`R3` and `R11` are READ-ONLY here** — the mint runs with `OVERLAY` and `MAP` redirected to scratch, so no cell, no map line, no CDN state and no engine output is written) | — | mint generator — production-path exercise | released 2026-08-16 | **free. GENERATOR NOW HAS PRODUCTION-PATH EVIDENCE; STATUS IS STILL BUILT, NOT PROVEN.** `evidence/mint-provenance/production_path_verdict.txt`. The real script ran with the real inputs and the emitted ancestry MATCHES an independent recomputation of the installed bytes — 11 artifacts, 2 differ, 9 identical, digests agreeing. Two free results: the address **re-derived to `cd137db6` exactly**, and the scratch cell's 11 engine artifacts are **byte-identical to the live published cell** (the compiler zip differs only in its `built` timestamp, the documented zip caveat). **WHAT IS STILL OWED FOR PROVEN:** a real mint that publishes — `0012`'s cycle needs one anyway, so read the emitted block there, recompute against the donor, require a match, then advance. **AND ONE COVERAGE LIMIT:** the real run hit only the *"2 of 11 differ"* branch; the all-identical and ADDED/ABSENT branches remain probe-only, so a clean clone with no substitutions has never been observed on the real path. Original claim text follows. **CLAIMED BEFORE THE RUN.** **THERE IS NO NEW PRODUCTION CELL TO MINT** — the engine tree is unchanged since `cd137db6`, `0012` is NOT applied (no `tpool_status` markers anywhere in the checkout), and the seven cell inputs all predate that mint, so a correct mint recomputes the same address and short-circuits on "map entry already present". Rather than invent an address, this runs the REAL script against a scratch destination with the REAL inputs, which also re-derives `cd137db6` as a by-product. **It cannot earn PROVEN and is not intended to** — nothing is published, and the repo's vocabulary reserves PROVEN for the real target. PROVEN is deferred to `0012`'s cycle, which needs a mint anyway |
| `packages/shorebird_cli` — `lib/src/commands/patch/patch_command.dart`, `test/src/commands/patch/patch_command_test.dart`; `selfhost/HANDOFF.md` verify block; `selfhost/evidence/analyzer-sdk-gap/` (new) (no `R-id`; **`R7` is NOT taken** — nothing here touches `route_b_producer.dart` or `analyze_coverage.dart`, so `analysisVersion` stays 8) | — | CI `Build shorebird_cli` failure — root cause + the SDK gap behind it | released 2026-08-16 | **free. THE DEFECT IS FIXED (`58d8cdd2`); THE SDK GAP IS RECORDED, NOT CLOSED.** CI's red job is `dart analyze --fatal-warnings lib test` failing on **one warning among 86 infos**: `patch_command.dart:666` on `main`, `unawaited_return_in_try_block`. **It is a real bug and THIS BRANCH HAS IT** at `patch_command.dart:1171` — `return patcher.assertUnpatchableDiffs(...)` un-awaited inside a `try`, so neither `on UserCancelledException` nor `on UnpatchableChangeException` can fire on a real async rejection. **The existing tests pass and cannot see it**: mocktail `thenThrow` raises SYNCHRONOUSLY inside the try. **AND THE LOCAL GATE CANNOT SEE IT EITHER** — local Dart is **3.12.2**, CI installs **3.13.0**, and 3.12.2 answers *"'unawaited_return_in_try_block' isn't a recognized lint rule"*. So `HANDOFF`'s "expect exit 0" is SDK-dependent and currently weaker than CI. **WHAT THE NEXT LANE INHERITS.** (1) **The SDK gap is OPEN** — `evidence/analyzer-sdk-gap/` states three ways to close it, cheapest first; until one is taken, **local analyze is a pre-flight and CI is the gate**, and `HANDOFF` now says so inline. (2) **The full 3.13 result on this tree is UNMEASURED** — no 3.13 SDK is installed here, and installing one should be side-by-side, since this machine's Dart comes from pinned Flutter caches the repo depends on. A hand scan found no sibling instances, which is a targeted check and not a substitute. (3) Branch `fix/unawaited-return-in-try` at `198e38c2` is **left in place** as a recoverable pointer to the same content; its worktree is removed. (4) Test-count expectation in `HANDOFF` corrected to **2514 pass / 2 skipped**, measured |
| `selfhost/HANDOFF.md` "Verifying your work" block, `PARITY.md` "Rule for updating this file", `selfhost/scripts/cspell_touched.sh`, `selfhost/evidence/cspell/`, `cspell.config.yaml`, `.gitignore` (no `R-id`) | — | cspell gate reconciliation + the anticipated-hash rule | released 2026-08-16 | **free. THE PER-CHANGE GATE IS LIVE; THE TREE-WIDE ONE IS NOT AND SHOULD NOT BE TREATED AS ONE.** `76cb1a04`. **The premise correction matters more than the tooling: CI HAS NEVER RUN the tree-wide command `HANDOFF.md` presented as its gate** — `main.yaml:30` → VGV `spell_check.yml@v1`, `modified_files_only` defaults **true** and is not overridden, and the job is **green** (run `31824444226`). **WHAT THE NEXT LANE INHERITS.** (1) Run `selfhost/scripts/cspell_touched.sh` before staging — it is a *per-change* gate (no new findings on lines you added), **not** "touched files clean", because the strict form is unusable while `PARITY.md` carries 123 inherited findings. (2) The tree-wide debt is **KNOWN RED at 1,770 / 153 files / 440 words**, recorded in `evidence/cspell/` with the raw output kept in diff-friendly form; it is a baseline, not permission to add. (3) **Promotion back to a tree-wide gate has four stated conditions**, and one is a negative control — reaching zero proves the count moved, not that anything watches it. (4) **UNVERIFIED, deliberately left open:** whether `cspell-action`'s incremental mode is per-file or per-line. The green run's head commit is not in this clone, so the decisive comparison was unavailable; the script documents its own rule instead of claiming CI-equivalence. If CI turns out to be per-file it is stricter, which is the safe direction |
| `selfhost/engine/route_b/mint_route_b_cell.sh`, `selfhost/engine/route_b/probes/mint_provenance.sh`, `selfhost/evidence/mint-provenance/` (no `R-id`; `R3` was NOT taken) | — | mint provenance generator — correctness defect | released 2026-08-16 | **free. BUILT AND HOST-VERIFIED; NOT PROVEN — NO CELL HAS BEEN MINTED WITH IT.** `1916a316`: cell ANCESTRY is now MEASURED (the installed cell digested against the donor file by file) instead of inferred from `--ios-artifacts` alone, which is what wrote `cd137db6`'s false line and `87130ae8`'s understated one. `probes/mint_provenance.sh` **16/16**, negative control = the frozen pre-fix wording, and a mutation run against a reverted product goes `PASS=10 FAIL=6` so the arms can fail. **THREE THINGS THE NEXT MINTER INHERITS.** (1) **`--dry-run` no longer previews the map block at all** — ancestry is measured from an installed cell and a dry run creates none; that is deliberate, not a regression. (2) The write path has never run end to end, so **read the emitted block on the next real mint before trusting it** — that mint is the proof. (3) **`87130ae8`'s map entry is still understated and was deliberately left** — it should be REGENERATED by this generator, never hand-patched, or the map collects a second manual exception. Nothing opened, built, minted, published or mutated; the probe digests real cells read-only and writes only under `mktemp` |
...
> | **3+** | **`G15` delivery — the POSITIVE half. PASSES 2026-08-14, and it un-blocks the design.** With `R3`'s dill fed into the cache directly, a marker placed in `_runMain` LANDS in a built release's `App.framework`; with the published dill it is ABSENT from the same app. So the `_runMain` candidate is **not refuted** and the repair below is **not speculative** — the mechanism works and only the publication path is broken. Evidence: `evidence/g15/gate3_positive_verdict.txt` | the marker is retained BY CONSTRUCTION (an unfoldable branch), because otherwise an absent marker could not be told from tree-shaking |
> | **3a** | **`G15` delivery repair — CLOSED 2026-08-15 on cell `87130ae841e92d3e8bd4ea1759fb3cb474a6b3e4`.** `audit_route_b_compiler.sh` → AUDIT CLEAN, check `4b` green, served bytes verified over the wire (`X-Overlay: hit`, dill `9f5a5f75…` ×8 vs the donor's `55e02ed8…` ×0 as a live control). Two script edits: `PSDK_REL` in `publish_ios_overlay.sh` derives the dill from `OUT` instead of `HOST_REL`, and `PSDK_ZIP` in `mint_route_b_cell.sh` installs the addressed dill after a byte-for-byte check — the second is REQUIRED because the mint's `cp -Rc` donor clone is what propagated the defect forward, so a publish-side fix alone can never reach a minted address. Existing cells untouched; `dart-sdk` deliberately still `R4`'s. **Gate 4 is no longer blocked on delivery.** Evidence: `evidence/g15/gate3a_verdict.txt` | historical detail below |
> | 3a′ | ~~**`G15` delivery repair**~~ — **DETECTION HALF DONE 2026-08-14; the PUBLICATION half is still owed and is a design decision, not a typo fix.** `audit_route_b_compiler.sh` check `4b` now compares the published `flutter_patched_sdk_product.zip`'s `platform_strong.dill` against the cell's own `flutter_platform_strong.dill` input, and it is proven in BOTH directions — it FINDs on cell `40eaa0ef` (`9f5a5f75…` vs `55e02ed8…`) and it OKs against a synthetic overlay where they agree, so it is not a check that cannot fail. The script's header had NAMED this concern (*"a platform dill that is not the one the release was compiled against"*) since it was written, while only ever checking the bundle against its own `PROVENANCE.txt` — self-consistency, not agreement with what a build downloads. **Expect every pre-repair cell to report this finding now; that is the check working, and suppressing it to restore a green line would re-hide the hole.** The publication half is unresolved because `HOST_REL`'s R4 default was chosen deliberately (R3's `host_release_arm64` dill fails the iOS AOT step with "Unexpected tag 4 (Field)"), so the fix is probably a `host_release_arm64_nodm` built IN `R3`, not a one-line path swap | one byte comparison closes a hole two sessions walked into. Only after the publication half can step 3's marker test mean anything |
> | **4** | **`G15` seam — MATRIX ANSWERED AT SOURCE LEVEL 2026-08-15: identity PASS, failure-coverage PASS for sync (ordering proven), async-completion ACHIEVABLE, success-semantics still OPEN.** Chain confirmed verbatim: `dart_isolate.cc:934` `Dart_GetField(library,"main")` → `:939` `InvokeMainEntrypoint` → `:900` `dart:ui _runMain` → `_startMainIsolate` → `_delayEntrypointInvocation` → `RawReceivePort` handler → `hooks.dart:410/412 userMainFunction(args)`. **Identity PASSES by construction** — the seam wraps the handle `main` was resolved to, not a message ordinal, which is what `OnHandleMessage` failed on. **Failure ordering is PROVEN, not inferred:** `onError` fires only for *unhandled* errors (`platform_dispatcher.dart:1434-1447`), the escape route is `RawReceivePort` handler → `tonic UnhandledError` (`dart_message_handler.cc:49`) → `_onError` (`hooks.dart:399`), and NO zone wraps the entrypoint (`isolate_patch.dart:58` — `RawReceivePort` "does not work with Zones"), so `main` runs in the ROOT zone and a lexical `try/catch` preempts the unhandled classification by definition. **Async completion is free:** the return value is discarded TWICE (`hooks.dart:410/412` and `isolate_patch.dart:292-299`) and `_ListStringArgFunction` already returns `Object?`, so capturing it needs no signature change — which is the direct fix for the "scheduling is not completion" defect that killed both prior seams. **Two obligations recorded:** the seam must catch→report→**RETHROW** or it silently removes the app's own error reporting, and a sync `try/catch` does NOT cover async `main`, so Q3 cannot be closed independently of Q4. **Q2 PRECOMMITTED THEN MEASURED 2026-08-15 — the seven shapes ran and the predictions held.** Probe `engine/route_b/probes/q2_main_shapes.dart`: S1/S5/S6 SYNC RETURN, S2 FUTURE COMPLETED, S3 FUTURE ERROR, S4 SYNC THROW at the boundary, S7 NO COMPLETION. **The contract is NOT falsified** — the precommit said gate 4 would reopen if the return/completion signal were unobservable in an ordinary shape, and it is observable in all of them, so capturing the discarded `Object?` suffices and only Q2 stays open. **Policy A's fatal cell is S7 and only S7**, exactly as named in advance. Two results worth more than the table: **S5 shows an app that handles its own error via `runZonedGuarded` is INDISTINGUISHABLE from a clean boot at the boundary** — correct, and it means the seam needs no logic to tell "handled by the app" from "never threw"; and S4's throw is caught AT the boundary, confirming the lexical position for the sync case. **Still unmeasured and not to be reported otherwise:** `onError` ordering (needs an instrumented engine — the source argument stays a source argument), first-frame reachability, and S7's "no completion in 250ms" which is a BOUNDED observation, not a proof of never — the same distinction the seam must honour, since treating *not yet completed* as *failed* would tombstone every slow boot. Evidence: `evidence/g15/q2_success_policy_precommit.md`, `q2_main_shapes_verdict.txt`. **C-vs-D ANSWERED AT SOURCE LEVEL 2026-08-15, and it inverts two assumptions.** (i) **"Earliest-of" is already implemented atomically** — `updater.cc:59-76` guards `ReportLaunchSuccess` AND `ReportLaunchFailure` with ONE `compare_exchange_strong` on `launch_completed_`, so two success reporters compose thread-safely for free. (ii) **C does NOT widen the window `0010` closed** — under earliest-of, `main` fires before any frame in S1/S2/S5/S6, so C widens nothing A already covers and only bounds S7, where A's window is INFINITE; on the false-backout axis C is strictly BETTER than A, not worse. (iii) **first-frame is reachable from dart:ui**, not only from C++: `hooks.dart:393 _drawFrame` runs on the UI thread in the ROOT ISOLATE (called from `platform_configuration.cc:86`), one file from the seam — whereas `Shell::OnAnimatorDraw` posts to the RASTER thread and would have split the seam across two languages and two threads. **D collapses into C** absent a liveness signal that is neither a frame nor a timer, and a timer is precommitted inadmissible. **THE REAL TRADE-OFF IS NOT WINDOW-WIDENING — IT IS FAILURE SUPPRESSION:** the shared latch means C banks success at the frame and then *cannot* report a later `main` failure (`runApp(); await throws;`), which A reports positively. A's cost falls on WORKING patches (S7 tombstoned), C's on BROKEN ones (one extra boot cycle) — and `0010`'s own stated asymmetry, *"the cheap error is to retry; the expensive one is to tombstone"*, argues for C. A third option (split the latch) is recorded and NOT recommended without reading `record_boot_success` and `mark_bad` together, because it changes the safety model rather than the seam. **Still to measure, not assume:** whether `_drawFrame` fires for a headless/add-to-app engine that never attaches a view — if not, C degrades to A there, which is acceptable but must be shown at the two-engine gate. Evidence: `evidence/g15/q2_c_vs_d_analysis.txt`. **STILL OPEN — the CHOICE ITSELF (A or C)**, because a synchronous return is NOT "the app booted" (`runApp` schedules the first frame, it does not draw it), so any definition banks earlier than "usable" and must say so out loud rather than smuggle it under "returned". **The hard case is now isolated to exactly one surface:** a valid `main` that never completes after becoming usable (`runApp` then `await` a long-lived Future) would never bank, and `0010`'s counter would tombstone a good patch after two boots. **No fallback chosen** — deliberately. Seven startup shapes to measure ON THE HOST are listed in the evidence. Evidence: `evidence/g15/runmain_seam_matrix.txt` | **UNBLOCKED ON DELIVERY AND PROVENANCE 2026-08-15.** Whether a `_runMain` change can reach the shipped program is no longer an open question: cell `87130ae8` serves the dill its address was computed over, and gate 3's positive half showed such an edit lands in the release AOT. **What is still owed before any code is written:** the four seam questions answered for `_runMain` (does the signal identify the actual `main` invocation structurally / what exactly constitutes successful completion / can app code suppress or alter failure reporting via `PlatformDispatcher.onError` / is async-`main` COMPLETION observed rather than mere scheduling), plus the non-returning-`main` case, which remains an explicit open design question — **do not invent a fallback before delivery of the primary is proven on device.** **Two boundaries that this unblocking does NOT extend:** `dart-sdk`/`frontend_server` provenance is unchanged and still `R4`'s, and cell `87130ae8` is **not release-proven** — first consumption needs cache INVALIDATION (never a written stamp), a normal warm/fetch, and positive confirmation that the `ios-release` Flutter binary EXISTS before cutting, because `isRouteBEngine` returns false on a merely-absent binary and the release still reports success. **Baseline caveat:** releases 89-95 and `killswitch_probe` `1.0.2+1`…`1.0.5+1` predate `G4.1c`'s injected defines, so gate 5 must re-cut its own baseline rather than reuse `1.0.5+1` — otherwise it carries two variables with the CLI as the silent one | build and prove the `_runMain` wrapper candidate, answering the same four questions of whatever fallback the non-returning-`main` case needs. The C++ `OnHandleMessage` seam FAILED three of the four; see the plan. **Now additionally gated on 3a:** the seam's Dart half (`dart:ui`) and native half (C++ reporter) would otherwise ship from two trees nothing holds in agreement |
...
> | **5** | **`G15` three-arm hardware gate — HELD 2026-08-15, and the boundary is deliberate.** The next iOS engine build and mint are technically available, but there is **no reason to spend the device cycle until `cps-ios` can produce a baseline whose persistence is independently observable** — a release that does not durably record makes every arm built on it unverifiable, which is the reachable-failure rule applied before the spend rather than after. Gate 4 stays **BUILT** at `0011`; it is not blocked, it is simply not worth proving against a baseline that may not exist. **THE PUBLISH RULE SET, all four parts, because dropping any one restores the failure:** (1) use the SAME ordinary user-OAuth CLI path releases 93/94 used — switching to `API_KEY` would cost comparability without resolving the mechanism, since the auth path is already refuted as the discriminator; (2) treat publish success as **PROVISIONAL** — the CLI's exit text, an HTTP `200`, and a same-session `releases list` are one client asking one server; (3) verify each release and patch immediately by **CONTENT READ** against the DB or API; (4) **if a write vanishes, PRESERVE THAT AS EVIDENCE — do not change auth state first.** Part 4 is the one that will be tempting to skip: a vanished baseline on this path, with 93/94's known-good history behind it, is a far better experiment than a deliberate re-login, and re-authenticating destroys the state that would explain it. Together these make the next publish an observation OF THE GATE PATH rather than an auth experiment. `cps-ios` was found on 2026-08-15 accepting, verifying and promoting releases/patches — HTTP 200/204, `Published` printed, `releases list` showing them in-session — and **the rows are absent from the database.** Verified on CONTENT reads, twice and across both organisations: max release **94**, max patch **60**, the app absent. **~~MECHANISM RETRACTED 2026-08-15:~~** this row previously said the plane was "opened, written through, and truncated with nothing committed", citing `code_push.db`'s frozen mtime and 0-byte `-wal`. **That is withdrawn.** A controlled `POST` against the live plane PERSISTED while mtime and size stayed byte-identical — so this bind mount does not propagate file metadata, and the metadata was never a witness to writes. Writes are not being discarded. **The disappearance is real and the cause is UNDETERMINED.** The general durability blocker is **CLEARED** — a controlled `POST` persisted and the plane is not discarding writes. Proposed lead: the vanished writes used a USER OAuth token, the persisting one used the `API_KEY`. **THIS LANE'S OWN DATA WEAKENS THAT LEAD, and it is recorded here because no other lane holds it:** releases **93 and 94 were cut through the ordinary `shorebird release ios` user-OAuth path** — stored credential plus `SHOREBIRD_JWT_ISSUER`/`AUTH_SERVICE_URL`, no `SHOREBIRD_TOKEN` anywhere — and **both persisted.** So "user OAuth" does not by itself separate the two populations; something that changed between 2026-08-14 23:36Z and the vanished writes does. Candidates, all untested: the CLI re-sync (`ba4e1c02` → `50ed19a7` → `98f84c17`), a token refresh, or an org/actor difference. **Do not narrow to the token type on the strength of the API_KEY probe alone** — it is one write on one path, and the counter-example is already on disk. Releases 89-94 and their artifacts ARE present, so gate 2 stands — **and the reason it survives is worth stating: this lane measured the ARTIFACT (stored runner zips pulled off the plane and unpacked), not the announcement.** A baseline re-cut for this gate must likewise be read back from the DB before anything is built on it. See the plane's own KNOWN GAP row. **PLUS ONE DELIBERATE ARM, added so it is hit on purpose rather than mid-gate:** patch a PRE-`injectedDefines` release with a replacement body that reads the environment, and confirm the refusal fires BY NAME with nothing uploaded. **The triggering shape is exact** (per the `G4.1c` lane, which measured both directions): a `String.fromEnvironment` / `int.` / `bool.fromEnvironment` constructor written LITERALLY IN THE REPLACEMENT SOURCE. A reference to one of the app's own consts does NOT trip it — that resolves through the import kernel — so an arm built from a const reference would pass and prove nothing. `G15`'s ordinary arms are unaffected: `bootProbe()`'s replacement is a throw or a literal flip, with no `fromEnvironment` anywhere | — good→success, throw→positive failure + backout, kill-before-signal→retry (re-earning `0010` at the later seam) | same repaired fixture and instrument; **every arm asserts marker movement before UI is read** |
...
that shape — the `mtime` inference and the OAuth-versus-`API_KEY` hypothesis were
not things anyone doubted enough to control, which is precisely why neither was
controlled. Certainty is what stops you building the control, so the controls you
have are always aimed somewhere else.
