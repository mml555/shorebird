...
| `R6` canonical fixture | H4 lane | `H4` steps 10-13 (obfuscated release) | **claimed 2026-08-14** | **HELD.** `airgap_app` currently `38.0.0+1`. Prior note stale on version: **free at version `23.0.0+1`.** `lib/main.dart` is left in its PATCH state (`value() => _secret`), not the release state — whoever resumes either reverts that line or treats it as the next patch's source. Previously: Added one private field the release never reads — being unread is the condition under test, since retention then depends entirely on P2's `--private-dill` enumeration. Version bumps to 23 with this release |
...
| `R8` `cps-ios` | H4 lane | `H4` steps 10-13 (obfuscated release + patch) | **claimed 2026-08-14** | **HELD.** Container up (healthy). Prior: **free.** Release 69 (`23.0.0+1`) and patch 1 published against it. Previously: Container up and healthy; one release and one patch will be published against it |
