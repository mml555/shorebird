...
| `selfhost/HANDOFF.md` "Verifying your work" block, `PARITY.md` "Rule for updating this file", `selfhost/scripts/cspell_touched.sh` (new), `selfhost/evidence/cspell/` (new), `cspell.config.yaml`, `.gitignore` (no `R-id`) | **HELD** | cspell gate reconciliation + the anticipated-hash rule | 2026-08-16 | **CLAIMED BEFORE THE FILES EXIST.** Doc + tooling only: no code, fixture, device, cell, CDN or control-plane state. **`HANDOFF.md:2295` presents a tree-wide `npx cspell selfhost packages/code_push_server/lib` as a verification gate. CI HAS NEVER RUN THAT COMMAND** — `main.yaml:30` calls VGV `spell_check.yml@v1`, whose `modified_files_only` defaults to **true** and is not overridden, so CI's gate is INCREMENTAL and is currently **green**. The tree-wide command is an approximation, which is the exact failure the block's own 2026-08-13 rewrite forbids. Measured baseline: **1,770 findings across ~150 files**, 348 of them in one generated `obfuscation_map.json`. No hash is cited here on purpose — see the rule this lane is adding |
