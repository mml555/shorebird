...
| `selfhost/engine/route_b/mint_route_b_cell.sh`, `selfhost/engine/route_b/probes/mint_provenance.sh` (new), `selfhost/evidence/mint-provenance/` (new) (no `R-id`; **`R3` is NOT taken** — nothing here opens the engine checkout, builds, mints, publishes or touches a cell) | **HELD** | mint provenance generator — correctness defect | 2026-08-16 | **CLAIMED BEFORE THE FILES EXIST**, per §17's 2026-08-13 lesson. `064115f9` corrected `cd137db6`'s provenance by hand; this fixes the GENERATOR that emitted it wrong. The sentence branches on `--ios-artifacts` alone (`mint_route_b_cell.sh:216-222`), so a `PSDK_ZIP` platform-dill substitution is structurally invisible to it and the map gets a false ancestry claim. **Deliberate §17 rule-3 deviation, stated rather than accidental:** this is code under `engine/route_b/` and rule 3 says worktree, but the decisive probe arm reads the REAL `cdn/overlay` cells, which are untracked and therefore absent from a worktree. Shared tree, explicit paths, stage last, `git show --stat` after. **Read-only against every cell** — the probe builds synthetic donor/cell dirs in `mktemp` and only ever digests the real ones |
