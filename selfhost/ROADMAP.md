...
| **P2** | **Widen the replacement ABI** — receiver **+ positional args**, then named/type args where required | **HOST-COMPLETE 2026-08-25; only the device specimen remains.** The widening was already built (patch `0006`, shipped in BOTH cells) and host-proven (`g37_param_abi.sh` 4/4). What was untested is the COMBINATION with a private receiver — now GREEN 4/4 (`p2_private_receiver_with_args.sh`). **PROVISIONALLY SELECTED, not corpus-selected.** Not corpus-selected: P1.5 produced no ranking. Selected on Phase 0's measured **6/10** plus an independently established architectural limitation, with P1 having just removed the former 9/10 private-scope blocker. Scored on its own fixtures before any broad compatibility claim |
