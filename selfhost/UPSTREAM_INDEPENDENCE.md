...
| 7 | `aot-tools.dill` | **Their AOT linker.** Emits `.vmcode` + link percentage | Both kill-gate spikes PASSED 2026-08-05 and **Route B was selected** — see [`ROUTE_B.md`](ROUTE_B.md), [`engine/route_b/README.md`](engine/route_b/README.md). **Steps 1–2 built 2026-08-09**: the patchable call form (`--patchable_static_calls`) and generated symbol retention, both proven on a macOS host harness. Never fetched: `pkg/aot_tools` does not exist in vanilla Dart, so it can only be rewritten. Cache-side already independent: a blocked fetch warns instead of dying | **Replaced** ✅ — **CORRECTED 2026-08-13.** This cell read *"**In progress** ◐ — steps 1–2 work on a host; target identity, packaging, CLI and the iOS port not started"*, which was true on 2026-08-09 and stale from 2026-08-11. Steps 3, 4 and 5 are struck through as DONE in [`ROUTE_B.md`](ROUTE_B.md), and the iOS port is device-PROVEN ([`PARITY.md`](PARITY.md) §2). The dependency is not *removed* — `pkg/aot_tools` still cannot be fetched or rebuilt — it is **obviated**: Route B needs no AOT linker, so nothing on the iOS path calls it. Kept as its own row rather than folded into items 1–6 because "we replaced it" and "we own it" are different claims |
