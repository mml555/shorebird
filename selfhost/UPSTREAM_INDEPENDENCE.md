...
route-protected under custom hashes, but it is generated by *Shorebird's own*
`shorebird/ci/internal/generate_manifest.sh`, so what sits under our hash today
is an upstream description of our hash — it advertises
`flutter_engine_revision: 83675ed2…` (neither our hash nor the pin) and lists
artifact overrides for Windows, x86 and arm ABIs we do not build.
