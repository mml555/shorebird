...
| config path — **now CLOSED** | client-side track selection via supported config was an open defect when this row was certified: the updater read `channel:` from the bundled `shorebird.yaml` while the CLI's parser rejected the key. **Fixed 2026-08-26** in two places — `ShorebirdYaml` + its generated parser (this repo) and `compileShorebirdYaml` (`selfhost/flutter/0001-shorebird-yaml-carry-channel.patch`), the second because the CLI fix alone left the key accepted but silently dropped from the shipped bundle. Verified end to end on device: `evidence/p6-auto-channel/VERDICT.md` |
