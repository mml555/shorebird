...
|---|---|---|
| **single** (default: SQLite + files in one volume) | `setup.sh --backup` / `--restore` | **CERTIFIED** — `br1_certify_single.sh`, 24 passed, 0 failed |
...
|---|---|---|
| single | 15 passed, **9 failed** | 24 passed, 0 failed |
...
would have to be guessed. Scale, additionally: both halves carry one
