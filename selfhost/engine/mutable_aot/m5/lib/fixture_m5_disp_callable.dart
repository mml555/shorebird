// MAOT-5 (#69) Task A -- dispatch matrix for the `callable` subject.
//
...
class Direct { @pragma('maot:mutable') String call(int x) => 'OLD:${x * 2}'; }
class DirectNew { @pragma('maot:mutable') String call(int x) => 'NEW:${x * 3}'; }
class DirectNew2 { @pragma('maot:mutable') String call(int x) => 'NEW2:${x * 7}'; }

class Virt { @pragma('maot:mutable') String call(int x) => 'OLD:${x * 2}'; }
class VirtSub extends Virt { @override String call(int x) => 'SUB'; }
class VirtNew { @pragma('maot:mutable') String call(int x) => 'NEW:${x * 3}'; }
class VirtNew2 { @pragma('maot:mutable') String call(int x) => 'NEW2:${x * 7}'; }

class Ifc implements Iface { @pragma('maot:mutable') @override String call(int x) => 'OLD:${x * 2}'; }
class IfcOther implements Iface { @override String call(int x) => 'OTHER'; }
class IfcNew { @pragma('maot:mutable') String call(int x) => 'NEW:${x * 3}'; }
class IfcNew2 { @pragma('maot:mutable') String call(int x) => 'NEW2:${x * 7}'; }

class SuperBase { @pragma('maot:mutable') String call(int x) => 'OLD:${x * 2}'; }
class SuperSub extends SuperBase {
  @pragma('vm:never-inline')
  String callSuper(int x) => super.call(x);
}
class SuperNew { @pragma('maot:mutable') String call(int x) => 'NEW:${x * 3}'; }
class SuperNew2 { @pragma('maot:mutable') String call(int x) => 'NEW2:${x * 7}'; }

class Other { @pragma('maot:mutable') String call(int x) => 'OTHERDECL:${x * 5}'; }

@pragma('vm:never-inline')
String siteDirect(Direct d, int x) => d(x);
@pragma('vm:never-inline')
String siteVirtual(Virt d, int x) => d(x);
@pragma('vm:never-inline')
String siteIface(Iface d, int x) => d(x);
@pragma('vm:never-inline')
String siteSuper(SuperSub s, int x) => s.callSuper(x);
@pragma('vm:never-inline')
String siteOther(Other d, int x) => d(x);
...
const _lib = 'lib:package:m5dispcallable/fixture_m5_disp_callable.dart';
...
const _decls = [
  'cls:Direct::method:call',
  'cls:Virt::method:call',
  'cls:Ifc::method:call',
  'cls:SuperBase::method:call',
];

void main() {
  final dumpDir = Platform.environment['MAOT_DUMP_DIR'] ?? '';
  _emit('input', input);
  _emit('retain', [DirectNew()(input), DirectNew2()(input), VirtNew()(input), VirtNew2()(input), IfcNew()(input), IfcNew2()(input), SuperNew()(input), SuperNew2()(input)].join('|'));
  ...
  for (var i = 0; i < _decls.length; i++) {
    _emit('install.${_forms[i]}.v2',
        _stage(_decls[i], 'cls:${_forms[i]}New::method:call', 2));
  }
  _observe('v2');
  for (var i = 0; i < _decls.length; i++) {
  ...
  for (var i = 0; i < _decls.length; i++) {
    _emit('install.${_forms[i]}.v3',
        _stage(_decls[i], 'cls:${_forms[i]}New2::method:call', 3));
  }
  _observe('v3');
  for (var i = 0; i < _decls.length; i++) {
