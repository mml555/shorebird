// MAOT-5 (#69) Task A -- dispatch matrix for the `operator` subject.
//
...
class Direct { @pragma('maot:mutable') String operator +(int x) => 'OLD:${x * 2}'; }
class DirectNew { @pragma('maot:mutable') String operator +(int x) => 'NEW:${x * 3}'; }
class DirectNew2 { @pragma('maot:mutable') String operator +(int x) => 'NEW2:${x * 7}'; }

class Virt { @pragma('maot:mutable') String operator +(int x) => 'OLD:${x * 2}'; }
class VirtSub extends Virt { @override String operator +(int x) => 'SUB'; }
class VirtNew { @pragma('maot:mutable') String operator +(int x) => 'NEW:${x * 3}'; }
class VirtNew2 { @pragma('maot:mutable') String operator +(int x) => 'NEW2:${x * 7}'; }

class Ifc implements Iface { @pragma('maot:mutable') @override String operator +(int x) => 'OLD:${x * 2}'; }
class IfcOther implements Iface { @override String operator +(int x) => 'OTHER'; }
class IfcNew { @pragma('maot:mutable') String operator +(int x) => 'NEW:${x * 3}'; }
class IfcNew2 { @pragma('maot:mutable') String operator +(int x) => 'NEW2:${x * 7}'; }

class SuperBase { @pragma('maot:mutable') String operator +(int x) => 'OLD:${x * 2}'; }
class SuperSub extends SuperBase {
  @pragma('vm:never-inline')
  String callSuper(int x) => super + x;
}
class SuperNew { @pragma('maot:mutable') String operator +(int x) => 'NEW:${x * 3}'; }
class SuperNew2 { @pragma('maot:mutable') String operator +(int x) => 'NEW2:${x * 7}'; }

class Other { @pragma('maot:mutable') String operator +(int x) => 'OTHERDECL:${x * 5}'; }
...
const _lib = 'lib:package:m5dispoperator/fixture_m5_disp_operator.dart';
...
void main() {
  ...
  for (var i = 0; i < _decls.length; i++) {
    _emit('install.${_forms[i]}.v2',
        _stage(_decls[i], 'cls:${_forms[i]}New::op:+', 2));
  }
  _observe('v2');
  for (var i = 0; i < _decls.length; i++) {
  ...
  for (var i = 0; i < _decls.length; i++) {
    _emit('install.${_forms[i]}.v3',
        _stage(_decls[i], 'cls:${_forms[i]}New2::op:+', 3));
  }
  _observe('v3');
  for (var i = 0; i < _decls.length; i++) {
