// MAOT-5 (#69) -- operator vertical slice.
//
// Same production mechanism as the instance-method and getter slices. No
// operator-specific machinery: if the shared trampoline/cell design cannot
...
class Alpha {
  @pragma('maot:mutable')
  String operator +(int x) => 'OLD:${x * 2}';
}

class AlphaNew {
  @pragma('maot:mutable')
  String operator +(int x) => 'NEW:${x * 3}';
}

class AlphaNew2 {
  @pragma('maot:mutable')
  String operator +(int x) => 'NEW2:${x + 7}';
}
...
class Beta {
  @pragma('maot:mutable')
  String operator +(int x) => 'BETA:${x * 5}';
}

// A statically wide receiver set, so the site is not devirtualised and can
// reach the megamorphic state.
class F1 { String operator +(int x) => 'F1'; }
class F2 { String operator +(int x) => 'F2'; }
class F3 { String operator +(int x) => 'F3'; }
class F4 { String operator +(int x) => 'F4'; }
class F5 { String operator +(int x) => 'F5'; }
class F6 { String operator +(int x) => 'F6'; }
...
const _lib = 'lib:package:m5operator/fixture_m5_operator.dart';
...
const _decl = 'cls:Alpha::op:+';
int _stage(String i, int v) =>
    _install(_c('$_lib::$_decl'), _c('$_lib::$i'), v, _c(_ns));
final _siteOut =
    Platform.environment['M5_SITE_REPORT'] ?? '/tmp/maot_operator_site.txt';
...
void main(List<String> args) {
  ...
  _emit('install.v2', _stage('cls:AlphaNew::op:+', 2));
  ...
  _emit('install.v3', _stage('cls:AlphaNew2::op:+', 3));
