...
#
# ONE ARTIFACT, TWO FILES. `dartaotruntime` and `dart2bytecode_aot.snapshot` are
...
Route B bytecode compiler — one logical artifact, three files.
...
echo
echo "NEXT: add a ShorebirdArtifact entry that resolves this from the RELEASE's"
echo "      engine revision — one resolution returning the whole set, never"
echo "      three independent lookups a caller could mix."
