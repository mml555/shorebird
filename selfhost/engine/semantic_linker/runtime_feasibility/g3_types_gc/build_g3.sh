...
# build_instrumented.sh -- SEMANTIC-LINKER-1 / G3. Build the same matched pair
# again, this time with the experiment-only execution-mode instrument applied.
#
# INTO NEW OUT DIRS, NEVER OVER G1's OR G2's. Those are accepted evidence and
...
LOG="$LANE/logs/build_g3_$(date +%Y%m%d-%H%M%S).log"
mkdir -p "$LANE/logs"
...
grep -q collectAllGarbageForTesting "$SRC/flutter/third_party/dart/sdk/lib/internal/internal.dart" \
  || { say "ABORT: the G3 GC instrument is not applied to the lane tree"; exit 1; }
say "GC instrument present on the dart:_internal surface"
...
step "ninja g3 OFF" nice -n 5 ninja -C "$OFF" -j 8 $TARGETS
# shellcheck disable=SC2086
step "ninja g3 ON"  nice -n 5 ninja -C "$ON"  -j 8 $TARGETS
...
say "--- G1/G2 artifacts, re-read ---"
for a in sl1_dm_off sl1_dm_on sl1_g2_off sl1_g2_on; do
