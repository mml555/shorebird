...
set -uo pipefail
SM="${1:?usage: run_final.sh <semantic-map-dir> [out-dir] [dart-git-dir]}"
G="$(cd "$(dirname "$0")" && pwd)"
OUT="${2:-$G}"
DART_GIT="${3:-${SM1_FINAL_DART_GIT:-}}"
# EXPORTED so a nested invocation -- family D runs this orchestrator against
# damaged evidence -- can regenerate the source record instead of deleting the
# copy it inherited and then failing for a reason unrelated to the arm.
export SM1_FINAL_DART_GIT="$DART_GIT"
REPO_ROOT="$(cd "$SM/../../.." && pwd)"
FREEZE="$SM/../semantic_linker/runtime_feasibility/g0_freeze/freeze_manifest.json"
SM="$(cd "$SM" && pwd)"
...
# The source record is regenerated too. Leaving a previous one in place would
# let stale source facts read as current -- and if the Dart tree is not
# available this run, the row must degrade to NOT_ESTABLISHED rather than
# quote yesterday's answer.
SRC_FACTS="$SM/final/evidence/source_facts.json"
rm -f "$SRC_FACTS"
...
echo "source tree $SM"
echo
echo "############ 0. SOURCE EVIDENCE FOR THE ROUTING MECHANISM ############"
cat <<'TXT'
  FINAL published five source-level routing facts as hard-coded prose. The
  facts were correct, which is beside the point: a material routing conclusion
  outside the digested evidence graph cannot be falsified by Family A, is not
  in the provenance manifest, and does not degrade when its evidence goes
  away. They are produced here instead, read from the FROZEN tree via
  `git show <effective_tree>:<path>` rather than the working copy, each with
  its git blob id.
TXT
echo
if [ -n "$DART_GIT" ] && [ -d "$DART_GIT" ]; then
  mkdir -p "$(dirname "$SRC_FACTS")"
  python3 "$G/lib/probe_source_facts.py" "$DART_GIT" "$REPO_ROOT" \
      "$FREEZE" "$SRC_FACTS"
  SRCFACTS_RC=$?
else
  echo "  NO DART TREE GIVEN. Pass it as the third argument or set"
  echo "  SM1_FINAL_DART_GIT. No source record is written, so"
  echo "  DYNAMIC_INTERFACE_VALIDATION_MECHANISM reads NOT_ESTABLISHED and the"
  echo "  routing says the mechanism is untraced -- which is the correct"
  echo "  answer when the source cannot be read, rather than a remembered one."
  SRCFACTS_RC=2
fi
echo
...
want 'the source record was produced' 0 "${SRCFACTS_RC:-1}"
want 'the source record is about the frozen tree' True \
     "$(python3 -c "
import json
d=json.load(open('$MATRIX'))['matrix'].get(
    'DYNAMIC_INTERFACE_VALIDATION_MECHANISM',{}).get('probe_values',{})
print(bool(d) and d.get('reads_frozen_tree') is True
      and d.get('facts_effective_tree') == d.get('frozen_effective_tree'))")"
want 'the routing mechanism is derived, not asserted' 0 \
     "$(python3 -c "
import json
# No routing line may cite a source location that is not a probe value.
d=json.load(open('$VERDICT'))
pv=json.load(open('$MATRIX'))['matrix'][
    'DYNAMIC_INTERFACE_VALIDATION_MECHANISM']['probe_values']
lines={str(pv[k]) for k in pv if k.endswith('_line')}
import re
cited=set()
for r in d['next_lane']:
    cited |= set(re.findall(r'\.(?:dart|sh):(\d+)', r))
print(len(cited - lines))")"
...
want 'RESULT.md is machine-generated, not authored' 1 \
     "$(count 'GENERATED by final/lib/gen_result.py' "$RESULT")"
want 'RESULT.md carries the computed verdict' 1 \
     "$(count "^    $(j "$VERDICT" "d['verdict']")" "$RESULT")"
...
# The header count and the lines under it must agree. The count was quoted as
# 33 in a report while the transcript said 29 -- a number stated instead of
# read. Now the transcript proves its own arithmetic.
NLINES=${#ASSERTIONS[@]}
NPASS=$(printf '%s\n' "${ASSERTIONS[@]}" | grep -c '^  pass  ')
NFAIL=$(printf '%s\n' "${ASSERTIONS[@]}" | grep -c '^  FAIL  ')
NNOTE=$(printf '%s\n' "${ASSERTIONS[@]}" | grep -c '^  note  ')
{
  echo
  echo "ASSERTIONS ($NLINES checked: $NPASS pass, $NFAIL fail, $NNOTE note)"
  if [ "$((NPASS + NFAIL + NNOTE))" != "$NLINES" ]; then
    echo "  ACCOUNTING BROKEN: $NPASS + $NFAIL + $NNOTE != $NLINES"
  fi
  printf '%s\n' "${ASSERTIONS[@]}"
  echo
