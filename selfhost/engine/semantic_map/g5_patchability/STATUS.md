...
## The `Base.work` dispatch experiment, and the predicate it licenses

Regenerated by [`run_dispatch_experiment.sh`](run_dispatch_experiment.sh),
which does **not** build its own subject: `run_route2.sh` builds
`app_release.aot` from `release3.dill` and banks the reader against it, and the
experiment consumes that exact file. Whole-AOT output is not byte-reproducible,
so a rebuild would silently hand the demonstration a different artifact from
the one the reader was banked against — the assertion
`the reader bank and this demonstration used one artifact` compares the two
SHAs directly.

### The subject was wrong twice, and both corrections are recorded

`app_s6.aot` was withdrawn because its input kernel could not be identified.
`app_s6r.aot` replaced it, built from `prepass3.dill` — and that was also
wrong: `prepass3.dill` is the **pre-pass** kernel, from which
`gen_dynamic_interface` derives `di3.yaml`, compiled **without**
`--dynamic-interface`. Its AOT runs and reports a build id, so it looks like a
release, but a patch against it fails at attach with
`Unable to find function DateTime.now` — the dynamic-module retention the
interface drives is absent. It was never the patchable release. The canonical
subject is now `app_release.aot`, built from `release3.dill`. Per-declaration
verdicts are identical across all three, so no verdict was traded away by
either correction.

### What the experiment establishes

`Base.work` is `NOT_INLINED`. Its replacement attaches — `IsInterpreted` 0→1,
`HasBytecode` 0→1 — and a C++ invoke of the target returns `PATCHED-w`, so a
stale call site is not a failed attach. On that one artifact, with that one
patch:

| call site | after patch | shipped shape |
|---|---|---|
| `viaDirect` — receiver constructed in place | `PATCHED-w` | pool-indirect, monomorphic entry |
| `viaVirtual` — polymorphic receiver | `OLD-w` | dispatch table, class-id indexed |
| `viaInlined` — helper inlined away | `OLD-w` | pool-indirect, **normal** entry |

So `NOT_INLINED` ≠ "every AOT caller observes the patch". The two statements do
not conflict: `NOT_INLINED` is about body copies, and this staleness is about
dispatch. Register roles come from source, not from reading the disassembly —
`PP = R27` and `DISPATCH_TABLE_REG = R21` (`constants_arm64.h:144-145`),
`EmitDispatchTableCall` emits exactly the
`AddImmediate` + `Call([R21, LR, UXTX, Scaled])` pair seen in `viaVirtual`, and
`UntaggedCode` places `entry_point_` at 8 and `monomorphic_entry_point_` at 16
(`raw_object.h:2014-2019`), so tagged `#0x7` and `#0xf` name those entries.

### The answer is asymmetric, and the third arm is why

* **Presence of staleness is mechanically detectable.** A dispatch-table call
  reads its entry point from the table indexed by the receiver's class id and
  never consults the callee's pool entry, so attaching a body cannot redirect
  it. This release carries 491 such shapes across 198 functions.
* **Absence is not determinable from call-site shape.** `viaInlined` has no
  dispatch-table call at all — only a pool-indirect one — and stayed stale.
  The two pool-indirect sites differ in which entry point they read, recorded
  as a *hypothesis* and explicitly not as a demonstrated cause.

[`lib/scan_dispatch_calls.py`](lib/scan_dispatch_calls.py) is therefore **shape
detection, not a redirectability classifier**: a dispatch-table shape is
sufficient to conclude a site is not redirected, a pool-indirect shape concludes
nothing, and the counts are matched shapes rather than a complete call-site
census. An earlier version of that file called pool-indirect "redirected",
which this experiment disproves. Pool offsets are reported as shape evidence
only — they are relative to each caller's own pool, and the same offset appears
in two functions where it cannot be the same object.

Attributing a dispatch-table site to the declarations it can reach needs the
precompiler's selector map, which is not in the ELF. No instrumentation for it
is proposed.
...
`NON_STATIC_DISPATCH_UNPROVEN` for a declaration known not to be static;
`STATIC_METADATA_UNUSABLE` when the metadata is absent, null, or not a boolean.
Two reasons rather than one, for the same reason the release pair is two:
"reached by instance dispatch" and "cannot tell whether it is" are different
facts, and merging them would let a missing input read as a measurement.

**`static == true` is not asserted safe.** It only avoids *this* refusal; the
inbound call mechanism for static declarations is not closed, and every arm
still carries `CALL_SITE_SHAPE_UNPROVEN`.

[`lib/falsify_dispatch_predicate.py`](lib/falsify_dispatch_predicate.py) drives
the real predictor over ten arms — instance method, getter, setter and operator;
`static` absent, null, the string `"true"`, and the integer `1`; a static
top-level function; and a field, which is not a replaceable callable. All ten
fail closed, and no arm is predicted patchable. Its positive control deletes
the predicate block: the eight arms that depend on it flip, and the two that
must not depend on it keep passing.

Still unauthorized and not done: predictor wiring, subset scoring,
Wonderous/LocalSend, and final G5 classification.
