...
// ---------------------------------------------------------------------------
// SM1-G5 PAIRED CALL-SITE ARMS.
//
// ONE patch target -- `Base.work` -- reached through several call-site shapes in
// the same release. The patch is identical for every arm, so any difference in
// observed effect is attributable to the SHAPE OF THE CALL SITE and to nothing
// else. That is the question #54 raises with its devirtualized and inlined arms.
//
// Every body routes through DateTime.now() for the reason this file already
// documents: a literal is constant-folded by TFA even under vm:never-inline.
class Base {
  @pragma('vm:never-inline')
  String work() => DateTime.now().millisecondsSinceEpoch >= 0 ? 'OLD-w' : 'X';
}

/// A second implementation, so the closed world holds more than one `work` and
/// the compiler MUST emit a dispatch. This is the open/virtual arm's premise.
class Other extends Base {
  @override
  @pragma('vm:never-inline')
  String work() => DateTime.now().millisecondsSinceEpoch >= 0 ? 'OTHER-w' : 'X';
}

/// 1. VIRTUAL through a receiver the compiler cannot bind.
@pragma('vm:never-inline')
String viaVirtual(Base b) => b.work();

/// 2. DEVIRTUALIZABLE: a concrete receiver constructed in place.
@pragma('vm:never-inline')
String viaDirect() => Base().work();

/// 3. INLINEABLE caller: the helper is deliberately NOT never-inline, so the
/// optimizer is invited to inline the call site away.
@pragma('vm:prefer-inline')
String _inlineHelper(Base b) => b.work();
