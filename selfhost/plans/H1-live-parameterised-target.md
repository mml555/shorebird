...
| field | value |
|---|---|
| status | **DISCHARGED 2026-08-14 — do not take it.** The readout gap named below is closed and `G3.7`, the only thing this order existed to unblock, is COMPLETE on device: release 38 carries `two(String a, int b)` live, called as `two('a', 7)`, with its result **displayed as well as beaconed**. Note how it closed — through `G3.7`'s own releases rather than through this order being executed as written, so its steps were never run in sequence and should not be cited as executed. Prior status, kept because the reclassification is the useful part: **PARTIAL — RECLASSIFIED 2026-08-13, premise falsified by the tree.** This read *"**NOT BUILT** — no live parameterised target exists today"*, which was true at `c0619d13` and stopped being true at `08a2f3cc` (2026-08-13 14:25). `RouteBThing.paramValue(String who)` carries its own source parameter at `airgap_app/lib/main.dart:147`, is `DateTime`-routed so its result cannot be folded, and is called at `:326` from `_routeBRead()` — which `initState` calls at `:307`, the live path. **What remains is the READOUT, not the target:** `_rbParam` is beaconed at `:376` but is not among `build()`'s seven `_row(...)` calls, and beacon-only is unobservable on this rig (`cps-ios` logs `/selfhost-beacon/state -> 403` with no query string, recorded at `:278-286`), so the value cannot be read at all today. Displaying it needs a row retired or the padding trimmed to fit 1334 px. Then the release and the `R1` launch. Original text follows, unedited: no live parameterised target exists today. This order's host checks earn **BUILT**; the release launch on `R1` earns **PROVEN** for the *reachability prerequisite only*. `G3.7`'s own row stays at the **BUILT** it already has (`PARITY.md:341`) until a patch flips `OLD`→`NEW` on device |
