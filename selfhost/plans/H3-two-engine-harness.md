...
| field | value |
|---|---|
| status | **DONE — CLOSED 2026-08-13, and `G15` itself has since PASSED ON DEVICE.** Every clause of the NOT BUILT text below is now false; it is kept because a reader who acted on it needs to know it changed. The two-engine host exists (`selfhost/fixtures/twoengine_app`, `2d906552`), `0007`'s three arming tests EXECUTE and pass via the slim target captured as `selfhost/engine/route_b/0008-g15-slim-arming-test-target.patch` (`13092e26`), and `e994a512` recorded the device pass: both engines arm Route B, 2 rbtrace records with distinct Functions, both markers `MARK-PATCHED` against a release body returning `MARK-REL`. **This order is complete — do not take it.** Superseded text: ~~**NOT BUILT** — no host in this repo creates two engines, and `0007`'s tests have never executed (there is no linked `out/host_release_arm64/shorebird_unittests` binary, only a `.o`). Per the Status vocabulary (`selfhost/PARITY.md:169-178`) a compile earns nothing; §14b's own words were "a corrected diagnosis plus a compile-verified fix — not a closed gate" (`:2635`)~~ |
...
3. **[MUTATES `R3`: `shell/common/shorebird/BUILD.gn`]** **Make `0007`'s tests runnable.** The blocker is exact: `shorebird_unittests` (`BUILD.gn:157-179`) compiles `patch_cache_unittests.cc` (`:161`) and deps `//flutter/runtime/shorebird:patch_cache` (`:175`), whose `patch_cache.cc:39` calls `Shorebird_ReadLinkHeader` — a symbol only Shorebird's private Dart fork defines. Two facts make a slim target look viable: `source_set("shorebird")` (`:134-150`) does not depend on patch_cache, and `flutter/runtime/BUILD.gn:123-129` gates that dep on `shorebird_use_interpreter`, declared `= is_ios` at `flutter/common/config.gni:36` and not overridden in `out/host_release_arm64/args.gn` — so false on this host build. So add, **inside the existing `if (enable_unittests) { … }` block (`BUILD.gn:152-180`)** — at top level `:shorebird_fixtures` is an undefined label and gn errors:
   ...
   **Then capture the edit as a patch, or it is lost.** `R3` is recreated from the series, so a BUILD.gn change that exists only in the tree evaporates and takes the committed evidence's meaning with it. `selfhost/engine/route_b/0008-g15-slim-arming-test-target.patch` ~~does not exist yet — creating it is part of this step.~~ **It EXISTS and is tracked as of `13092e26`** — this step is done; the capture happened. Note the series' prefixes are inconsistent (`0003` uses `a/engine/src/flutter/…`, `0007` uses `a/shell/common/shorebird/…`): state in the patch header which root you generated from, and verify by **content** rather than by `git apply` exit code — `create_checkout.sh:58` says why.
