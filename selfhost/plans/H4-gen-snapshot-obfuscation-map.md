...
| field | value |
|---|---|
| status | **NOT RUNNABLE** — not "unrun". The flag does not exist in our Dart tree, so the G4.3 obfuscated-matching arm has no harness. Building the harness is this order. Nothing here is PROVEN until step 12 |
...
2. **Write claim reports tree health, not just ownership.** Before the engine build, `bash /Users/mendell/shorebird/selfhost/engine/dart_patches.sh --dest /Volumes/build/route-b/flutter/engine/src/flutter/third_party/dart --verify` → `OK: all 4 patches applied on the pinned base.` That output *is* the GREEN you report in the claims row. Anything else is RED and you stop.
...
3. **Should the new flag be added to `routeBUnfingerprintableOptions`?** It is a patch-side input the release never has, so it is arguably not part of the *effective configuration* at all — but `_verifyBuildConfigAgrees` (`ios_patcher.dart:754-793`) reads the same arg list. Decide before step 11 or the first patch will tell you at the worst moment.
