...
| [`G15`](G15-crash-backout-and-restart-required.md) | the two symptoms `0007` did not touch — a Dart-phase crash backs the patch out, and restart-required becomes true rather than inferred. Two of §15's seventeen gates | **HALF CLOSED 2026-08-19/20 — do not re-take the backout half.** A Dart-phase FAILURE (not crash: it hangs) backs the patch out, and an AMBIGUOUS pre-success death now retries instead of tombstoning a healthy patch. Both device-proven; §15's gate ticked with corrected wording. Lifecycle behaviour is **FROZEN** ([`../MEASUREMENT_MODE.md`](../MEASUREMENT_MODE.md)) — the remaining question is the threshold VALUE, which needs fleet data, not engineering. **Still open: restart-required truthfulness.** Prior status below. **DESIGN 2026-08-14, earns nothing.** Reworked after an adversarial review refuted the first one BEFORE it was built: moving success to first frame would have let any pre-first-frame death (swipe-away, watchdog, jetsam) tombstone a GOOD patch. The rework banks success at the earliest *sufficient* point instead of the latest safe one, and counts consecutive failures rather than concluding from one. No code exists | `R3` + mint + `R1` when built; nothing today | **R1** for the gate | yes |
