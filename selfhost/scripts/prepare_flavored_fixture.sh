...
# like the right one.
mkdir -p "$GEN_DIR"
# A LOCALHOST base_url IS DEVICE-UNREACHABLE, and it fails SILENTLY.
#
# `localhost` on the phone is the phone. An app built with it never reaches the
# control plane at all: the release succeeds, the patch publishes, the device
# shows the BASELINE, and nothing anywhere reports an error. Measured 2026-08-26
# on the P6 flavor arm, which read V1/Foo after a patch was live and 100% rolled
# out -- state.json had no patch and an empty event queue.
#
# So localhost is fine for the HOST-ONLY yaml this script writes without an
# --app-id, and is refused once an app id is present, because an app id means a
# real release that a device is expected to fetch. `airgap_app`, the fixture with
# banked device evidence, uses the host's LAN address for exactly this reason.
if [[ -n "$APP_ID" && "$BASE_URL" == *localhost* && "${ALLOW_LOCALHOST:-0}" != 1 ]]; then
  die "base_url is $BASE_URL but --app-id was given.

A device cannot reach localhost -- that address is the phone itself -- and the
failure is silent: the release publishes, the patch publishes, and the app keeps
showing the baseline with no error anywhere.

Pass the host's LAN address, e.g.:
  --base-url http://$(ipconfig getifaddr en0 2>/dev/null || echo '<host-ip>'):18080

Set ALLOW_LOCALHOST=1 only for a host-only build that no device will fetch."
fi
