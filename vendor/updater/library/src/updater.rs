...
/// Verifies a patch signature using the PRODUCTION verifier.
///
/// A thin passthrough to `cache::signing::check_signature`, exposed only under
/// `test-hooks` so a harness can drive the real verification code. It exists
/// because the Dart CLI produces the two values this function consumes -- a
/// base64 signature and a base64 DER public key -- and nothing previously
/// exercised that CROSS-LANGUAGE boundary: the Dart tests sign and verify with
/// Dart, and the Rust tests verify constants generated by hand.
///
/// Deliberately not a production C symbol, not `pub` on `cache::signing`, and
/// not a reimplementation: the point is that the bytes the CLI emits reach the
/// same `ring` call an on-device patch install would reach.
#[cfg(any(test, feature = "test-hooks"))]
pub fn testing_check_signature(
    message: &str,
    signature: &str,
    public_key: &str,
) -> anyhow::Result<()> {
    crate::cache::signing::check_signature(message, signature, public_key)
}
#[cfg(test)]
